...
**Судятся только листовые пространства имён.** Пространство имён, у которого в запуске есть вложенное пространство имён, на уровне пространства имён не сообщается, каким бы ни был его CBO: его число берётся по всему поддереву и растёт вместе с ним, поэтому пороги его не описывают. Это касается и пространства имён, которое объявляет собственные классы рядом со вложенными пространствами имён, -- его CBO всё равно по поддереву. Значение при этом публикуется (`coupling.cbo` на пространстве имён в `--format=metrics`), а его классы, как обычно, судятся на уровне класса. Какие пространства имён листовые, зависит от того, что проанализировал запуск: при суженном запуске пространство имён, чьи вложенные пространства имён в него не вошли, -- лист.
...
**Уровень пространства имён** (включён по умолчанию, только листовые пространства имён, требует минимум 3 класса в пространстве имён):
