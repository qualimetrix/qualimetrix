...
| Unreachable Layer     | `architecture.unreachable-layer`   | Info                                   | enabled (fires only with `architecture.layers`) | One Info diagnostic per declared layer whose patterns matched zero classes. Catches a broader pattern earlier in the order silently swallowing a later layer. Info-only runs exit 0 by default; use `fail_on: info` to enforce.                                                                                                                          |
| Potential Shadow      | `architecture.potential-shadow`    | Info                                   | enabled (fires only with `architecture.layers`) | Evidence-based detection of layers that silently steal classes from later, narrower layers. One diagnostic per (assigned, shadowed) pair. Info-only runs exit 0 by default; use `fail_on: info` to enforce.                                                                                                                                              |
| Empty Template        | `architecture.empty-template`      | Warning                                | enabled (fires only with template layers)       | One Warning per template layer that expanded to zero concrete instances — silently disables the policy attached to it. Typical causes: typo in the template pattern, every candidate excluded, or single-segment `{var}` where `{var:**}` is needed. Default `fail_on: error` does not block; raise to `fail_on: warning` to gate CI on empty templates. |
