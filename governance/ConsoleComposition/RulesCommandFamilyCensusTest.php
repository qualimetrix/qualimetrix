...
/**
 * Census guard for the `rules` command's producer/family surface against the
 * production container. Split off from `RulesCommandWiringTest` (which keeps
 * the behavioural refusal case): both methods here quantify over "every rule
 * the real container registers", not over fixed input.
 */
#[CoversClass(RulesCommand::class)]
#[CoversClass(RuleCompilerPass::class)]
final class RulesCommandFamilyCensusTest extends TestCase
{
    #[Test]
