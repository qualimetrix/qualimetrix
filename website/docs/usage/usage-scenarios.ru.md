...
- Используйте `--fail-on=error`, чтобы предупреждения не приводили к падению сборки. Это даёт возможность постепенного внедрения.
...
- Рассмотрите использование `--fail-on=error` в конфигурации хука, чтобы пропускать предупреждения
...
Нарушения отображаются как аннотации warning/error непосредственно на изменённых строках в diff PR. Дополнительная настройка не требуется.
...
| ---------------- | ------------------------------ | ----------------------------------------- |
| CI/CD            | `github` или `sarif`           | `--fail-on=error`, `only_rules` в конфиге |
