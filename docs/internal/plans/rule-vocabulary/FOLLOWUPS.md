...
- **Три вещи оттуда — не диагностика, а дефекты**, и они отдельны от этой
  ...
  1. **`suppress_namespace_channels` валидируется и не гасит.** Ключ,
     адресующий не канал, отвергается кодом 3 и внятной фразой — то есть
     валидация настоящая. Но и при верной форме `{complexity.ccn: [VerNs]}`
     находка канала в этом неймспейсе остаётся на месте. Соседние
     `suppress_namespaces` и `suppress_paths` того же правила на том же
     пробнике гасят её обе.
