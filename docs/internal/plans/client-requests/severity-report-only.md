...
вторым зубом»; пример больше не годится. **Компенсация приземлилась не нижней границей severity, а жёстче**
(подложка исполнена 2026-08-19, `f9039a45`): severity этих каналов зафиксирована, три per-diagnostic ключа
удалены, а `ConfigurationError` не сравнивается с `fail_on` вовсе и гейтит безусловно. Формулировки про
«нижнюю границу» ниже по тексту читать как исторические. Обещание формулируется как «Info не гейтит
...
нет. Подложка закрыла это не границей severity, а безусловным гейтом мимо `fail_on`.
**Учесть при написании:** `potential-shadow` 2026-08-19 сужен и на легальной идиоме «narrow before broad»
молчит, так что как пример «Info-канала» он больше не годится вдвойне. Писать этот план надо в мире, где
