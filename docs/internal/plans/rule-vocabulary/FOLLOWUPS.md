...
`reset()`, поэтому политика не хранит ни одной директивы, и обе половины аудита
отвечают пустым списком — не «нечего сказать», а «нечего спросить». Для
`auditDirectiveUsage()` это правильно (канал принадлежит правилу), для будущей
команды — нет: `qmx directives --disable-rule=annotation.directive` отчитается
пустотой вместо отказа.

- **Цена:** отчёт, неотличимый от «директив в дереве нет».
- **Что закроет:** П4 — либо отказ команды с внятным сообщением, либо
  подготовка директив независимо от enablement правила.

### Форма отчёта — единственное утверждение П2 без мутационного свидетеля

Три раунда фиксов довели число мутаций до восемнадцати, и каждая красит свой
случай.
Единственный случай, которого не двигает ни одна, —
`itReportsWhatTheRunMeasuredAlongsideTheVerdicts`: он утверждает форму отчёта —
полноту охвата, число проанализированных файлов и непустоту произведённого.

- **Цена:** утверждение проверено только тем, что оно сегодня истинно.
- **Что закроет:** П4, где эти поля станут шапкой отчёта команды и получат
  свидетеля в виде сравнения вывода.
...
- **Что закроет:** П4 — формулировка строки отчёта, отделяющая утверждение от
  ярлыка; либо, если понятие направления когда-нибудь появится у слоя правил,
  разделение вердикта надвое.
