 ...
 * that, and it left 39 of 166 namespaces out of the average to get it.
 *
 * The property is measured, not spelled: a rollup moves when a child gains a
 * type it never mentions, an own-scope value does not. The key's name is not
 * evidence — `-own` is a convention a new definition can satisfy while carrying
 * a subtree number.
 *
 * Two channels, because one would be its own oracle. Which keys are folded is
 * read from the declarations in `src/`; whether each holds is read from two
 * runs. A declaration this file cannot read is refused rather than skipped: the
...
final class NamespaceFoldCountsEachDeclarationOnceTest extends TestCase
{
    ...
    #[Test]
    public function itLeavesAParentUntouchedWhenAChildGainsAType(): void
    {
        $before = self::runOver(false)->get(SymbolPath::forNamespace('FoldFixture'));
        $after = self::runOver(true)->get(SymbolPath::forNamespace('FoldFixture'));
        ...
        foreach (self::foldedKeys() as $key) {
            self::assertSame(
                ...
                $after->get($key),
                \sprintf(
                    '"%s" is folded to the project, and the parent\'s value moved when a child gained an'
                    . ' unreferenced type — so it is a subtree rollup and the fold counts the child twice.'
     ...
     * namespaces carrying the key, and both fixture namespaces declare a type.
     */
    #[Test]
    public function itFoldsEveryNamespaceThatCarriesTheKey(): void
    {
        $project = self::runOver(false)->get(SymbolPath::forProject());

        foreach (self::foldedKeys() as $key) {
            self::assertSame(
                2,
                $project->get($key . '.count'),
                \sprintf('"%s" is carried by both fixture namespaces; the fold reached a different number', $key),
            );
        }
    }
    ...
    /**
     * A parent namespace that declares a class of its own, and a child that
     * declares one too — the shape the leaf-only fold used to drop. With
     * `$childGainsAType` the child gains an interface nothing references, which
     * moves the parent's subtree abstractness and nothing the parent itself
     * declares.
     */
    private static function runOver(bool $childGainsAType): MetricRepositoryInterface
    {
        $variant = $childGainsAType ? 'grown' : 'plain';
        ...
        if (!mkdir($root . '/Child', 0o777, true) && !is_dir($root . '/Child')) {
        ...
        self::write($root . '/Child/Reached.php', <<<'PHP'
            <?php
