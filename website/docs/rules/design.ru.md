...
!!! info "Отклонение от оригинальной спецификации"
    Оригинальный LCOM4 (Hitz & Montazeri, 1995) определяет рёбра только через общий доступ к свойствам. AIMD расширяет это рёбрами по вызовам методов (`$this->method()`), следуя стандартному подходу современных инструментов (SonarQube, JDepend). Без этого расширения хорошо декомпозированный класс, обращающийся к свойствам через геттеры, выглядел бы как имеющий плохую связность.
