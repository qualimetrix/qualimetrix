...
Путь, который вы называете и который сам является каталогом `vendor`, `node_modules` или `.git` (`bin/qmx check lib/vendor`), останавливает прогон ошибкой конфигурации: Qualimetrix никогда не заходит в такой каталог, и прогон не проанализировал бы ничего. Назовите файл или каталог внутри него (`bin/qmx check vendor/acme/`); каталог `vendor` внутри названного пути по-прежнему пропускается.

Каталог, который вы называете — здесь или в `paths:` файла `qmx.yaml`, — и который убирает ваш собственный `exclude:` или `--exclude`, останавливает прогон ошибкой конфигурации (код 3) до начала анализа; сообщение называет путь и убравший его селектор: иначе прогон завершился бы успешно, так и не заглянув внутрь. `directives`, `baseline:generate`, `baseline:update`, `baseline:cleanup` и `baseline:explain` отклоняют его так же. Файл, названный внутри исключённого каталога, по-прежнему анализируется, как и каталог под тем, который убирает селектор `exact:`: `exact:` не действует ниже названного им каталога. Путь, определённый по `composer.json` и исключённый вами, пропускается молча: там исключение делает ровно то, ради чего написано.

Если пути не указаны, Qualimetrix автоматически определит их по всем путям, которые объявляет секция `autoload` вашего `composer.json`, — корням `psr-4` и `psr-0`, записям `classmap` (шаблон `*` раскрывается в подходящие каталоги) и записям `files`. По этим же путям Qualimetrix судит, охватил ли прогон весь проект, поэтому прогон без путей всегда считается охватившим его. Запись, которая сама является каталогом `vendor`, `node_modules` или `.git` или лежит внутри него (Qualimetrix туда никогда не заходит), не входит ни туда, ни туда и называется в предупреждении: сторонний код не анализируется как код вашего проекта. Объявленный путь, которого нет на диске, останавливает прогон ошибкой конфигурации. `autoload-dev` не включается: тестовый код анализируется, только когда его путь назван явно (`bin/qmx check src/ tests/` или `paths:` в `qmx.yaml`), или когда вы считаете его частью проекта через [`--include-autoload-dev`](#--include-autoload-dev).
...
Значение, убирающее каталог, названный вами как путь, вместо этого
отклоняется — см. [Аргумент paths](#аргумент-paths).
...
Существующий файл пишется на месте, как это делает `>` в оболочке: он
сохраняет inode, владельца, права и жёсткие ссылки, и файл, смонтированный в
контейнер по одному, тоже записывается. Символическая ссылка разрешается до
файла, на который указывает, и остаётся ссылкой; так же на месте остаются
`/dev/null`, именованный канал и подстановка процесса вроде
`>(gzip > report.json.gz)`. Имя, под которым ещё ничего нет, создаётся, в том
числе через висячую символическую ссылку; если запись затем не удалась,
созданный ею файл удаляется (созданный через ссылку остаётся). Запись,
оборвавшаяся посреди существующего файла, оставляет его записанным частично.
По каким ссылкам идти, решает ядро: на Linux с `fs.protected_symlinks` ссылка,
оставленная другим пользователем в общем каталоге вроде `/tmp`, отклоняется,
как её отклоняет `>`. В потокобезопасной (ZTS) сборке PHP такая ссылка на
существующий файл всё ещё разрешается.

`/dev/stdout`, `/dev/stderr`, `/dev/fd/N` и `/proc/self/fd/N`, записанные
ровно так, пишутся через сам поток — терминал, канал или файл. Другое написание
того же потока, например `/proc/thread-self/fd/1` или символическая ссылка на
`/dev/stdout`, открывается по пути, и на Linux это не удаётся, когда поток —
канал: используйте одно из четырёх написаний.

qmx проверяет до прогона то, что может; окончательно судит сама запись.
Каталог или имя, оканчивающееся на `/`, существующий файл, недоступный для
записи, новое имя в каталоге, которого нет или в котором нельзя создать файл,
дескриптор, который процесс не держит открытым, а на Linux и дескриптор,
открытый только на чтение, отклоняются с кодом 3 до начала анализа.
Символическая ссылка, которая не ведёт к файлу, который qmx может создать
(висячая в несуществующий каталог, петля, ссылка, по которой ядро отказывается
идти), отклоняется самой записью с кодом 3 после прогона, как и любая запись,
не удавшаяся уже тогда.
...
Не может быть объединён с `--format-opt=violations=N` (числовой лимит) и с `--detail`/`--detail=N` (ограниченный список; `--detail` без значения — это лимит 200): каждое такое сочетание отклоняется с кодом 3 до анализа, а не решается молчаливой победой одного из двух. Совместное использование `--all` с `--format-opt=violations=all`, `--detail=all` или `--detail=0` допустимо (они синонимы).
...
Взаимоисключающий с `--class`. Отклоняется с кодом 3 до анализа под
`--format=gitlab` или `--format=checkstyle`, в том числе когда формат выбран
файлом конфигурации: их потребители читают каждую запись как находку, поэтому
ни один из них не может сказать, что отчёт — [частичный вид](output-formats.ru.md#summary-по-умолчанию).
...
Взаимоисключающий с `--namespace`. Под `gitlab` и `checkstyle` отклоняется так
же, как [`--namespace`](#--namespace).
...
Записывать лог прогона в файл, по одной JSON-записи на строку:

```bash
bin/qmx check src/ --log-file=qmx.log
```

Файл дописывается, недостающий каталог создаётся. Путь, в который нельзя
записать, — каталог, который не удаётся создать, или файл, который не удаётся
открыть на дозапись, — отклоняется с кодом 3 до начала анализа, и сообщение
приводит причину, названную системой. Пустое или пробельное значение —
`--log-file=`, как его пишет незаданная переменная в `--log-file=$LOG`, —
тоже отклоняется с кодом 3, а не читается как «лога нет»: чтобы не писать лог,
не указывайте опцию. Опцию принимает только `check`.

### `--log-level`

Установить минимальный уровень строк лога — для файла `--log-file` и для
консоли:
...
отвергается с кодом 3, а не откатывается к значению по умолчанию.

| Задано                 | Консоль                                                 | `--log-file` |
| ---------------------- | ------------------------------------------------------- | ------------ |
| ничего                 | `warning`; с `-v` — `info`; с `-vv` — `debug`           | `info`       |
| уровень, с `-v` и выше | этот уровень                                            | этот уровень |
| уровень, без `-v`      | этот уровень, если он строже `warning`, иначе `warning` | этот уровень |

Без `-v` уровень может только сузить консоль: `--log-level=error` скрывает
предупреждения, а `--log-level=debug --log-file=qmx.log` пишет подробный файл и
оставляет терминал таким же тихим, как был. `-q` заглушает консоль при любом
уровне; файл свои строки всё равно получает.
...
Цель, в которую нельзя записать, отклоняется с кодом 3 до начала анализа по
тем же правилам, что у [`--output`](#--output--o): каталог или имя,
оканчивающееся на `/`, файл в каталоге, которого нет или который недоступен
для записи, или файл, недоступный для записи. Пустое `--profile=` отклоняется так же. Запись, не
...
нельзя создать файл, отклоняются с кодом 3 до чтения первого файла;
существующая цель — файл, символическая ссылка, `/dev/stdout`, именованный
канал — пишется на месте.
