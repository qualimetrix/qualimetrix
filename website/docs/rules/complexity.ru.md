...
AIMD использует расширенный вариант цикломатической сложности, иногда называемый **CCN2+**. Помимо стандартных точек принятия решений (if, elseif, while, for, foreach, case, catch, &&, ||, ?:), AIMD также учитывает:
...
!!! note "Сравнение с другими инструментами"
    Из-за этих дополнительных точек принятия решений AIMD будет показывать **более высокие значения CCN**, чем phpmd или pdepend, для кода, использующего null coalescing или nullsafe-операторы. Это не баг — это отражение более строгого определения сложности. Разница особенно заметна в коде с цепочками выражений `??`.
...
AIMD следует Немеху (1988) с расширениями для PHP:
...
**Выражения `match`:** AIMD использует **аддитивный** подход, согласованный с оригинальной формулой Немеха для `switch`:

```
NPath(match) = 1 + sum of NPath(each arm body)
```

Некоторые другие инструменты (в частности pdepend) используют мультипликативный подход для `match`, что может давать экстремальные значения (миллионы) для методов с большими `match`-выражениями. Аддитивный подход AIMD даёт практичные и применимые значения.
...
WMC вычисляется как сумма [цикломатической сложности](#цикломатическая-сложность) всех методов класса. Поскольку AIMD использует вариант CCN2+ (который учитывает `??` и `?->` как точки принятия решений), значения WMC будут соответственно выше, чем у других инструментов.
