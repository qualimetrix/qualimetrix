...
use LogicException;
use Qualimetrix\Core\Metric\BaseCollectorInterface;
...
final readonly class DerivedCollectorExecutionPlan
{
     ...
     * @param array<string, BaseCollectorInterface> $collectors
     */
    ...
    private function participatesAtLevel(BaseCollectorInterface $collector, SymbolLevel $level): bool
    {
        foreach ($collector->getMetricDefinitions() as $definition) {
            if ($definition instanceof MetricDefinition && $definition->collectedAt === $level) {
                return true;
