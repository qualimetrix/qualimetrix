...
механизм, повышающий пробитую базлайнимую находку до `Error` (`Baseline README:128`), включая
occurrence-каналы архитектуры. Это регрессионный гейт против принятого состояния,
ортогональный severity; он остаётся и после среза. Обещание формулируется как «Info не гейтит
