...
Родитель вне анализируемого пути разрешается **чтением** его файла, а не
загрузкой. Файл находится по composer-установке самого анализируемого проекта
--- его `composer.json`, секциям psr-4 из `vendor/composer/installed.json` и
сгенерированному classmap'у, --- после чего разбирается объявление `extends`.
Код анализируемого проекта при этом не исполняется.

Поэтому глубина за пределами вашего кода --- это то, что объявляет установка
вашего проекта. Если зависимости не установлены или класс лежит в пакете,
которого установка не размещает, цепочка на этом обрывается, и сообщается та
глубина, которую удалось пройти: это нижняя граница, а не измерение. Оборванная
цепочка и класс, у которого родителя действительно нет, по-прежнему дают одно и
то же число.

DIT считается для классов. Интерфейсы, трейты и перечисления его не получают и
не входят в популяцию, по которой усредняются агрегаты уровня пространства имён
и проекта. Эта популяция --- именованные классы анализируемого пути, то есть то
же, что считает `size.class-count`, пока каждое имя класса объявлено один раз.

Агрегаты обобщают те же per-class значения, которые публикует отчёт, включая
цепочки наследования через файлы: класс, родитель которого объявлен в другом
файле, разрешается по графу зависимостей, и `design.dit.avg`, `.max` и `.p95`
считаются по разрешённым глубинам.

Подкласс, объявленный более чем в одном файле, считается один раз: NOC считает
различные имена потомков, а не рёбра `extends`.

NOC публикуется для той же популяции, что и DIT, -- именованных классов, -- поэтому
оба агрегата делятся на одно и то же число. Интерфейс, трейт или enum не несут
`design.noc` вовсе, даже 0, а `interface B extends A` не делает `B` потомком `A`:
иерархия интерфейсов -- это иерархия контрактов, а не наследование классов.
...
Родитель вне анализируемого пути разрешается чтением его файла, а не загрузкой.
Файл находится по composer-установке **вашего** проекта --- его `composer.json`,
psr-4 из `vendor/composer/installed.json` и сгенерированный composer'ом classmap
--- после чего разбирается его `extends`. Ничего из анализируемого проекта не
исполняется, и ответ берётся из вашей установки, а не из пакетов, случайно
оказавшихся рядом с `qmx`.

Поэтому глубина за пределами вашего кода --- это глубина, которую объявляет ваша
установка. Там, где обход не смог дойти до корня --- зависимости не установлены,
пакета с предком нет, его файл не разбирается --- сообщается то, что удалось
пройти. Обычно это пол, и класс с оборванным обходом сообщает значение того же
вида, что и класс, у которого родителя действительно нет.

В отчёте они неразличимы, но прогон об этом говорит. `qmx` пишет одно
предупреждение: сколько цепочек не доведено до корня и где остановился обход:
...
Формулировка намеренно узкая, потому что «пол» верен не для всякого случая.
Петля наследования между двумя внешними классами даёт длину петли, а это вообще
не глубина; кроме того, обход сдаётся после 64 шагов. Поэтому предупреждение
говорит, где остановился обход, и оставляет истолкование вам.

Оно идёт в поток ошибок, поэтому `--format=json` и остальные машинные форматы
остаются разбираемыми. `-q` его заглушает, и `--log-level=error` тоже --- с `-v`
и без него; без `-v` заданный уровень может сделать консоль только тише
`WARNING`, но не громче. Это высказывание
о том, что смог пройти прогон, а не о вашем коде: анализировать проект без
`composer install` --- нормально, и вот чего это стоит.

Глубина, выводимая из самого анализируемого пути, от этого не зависит: когда вся
цепочка внутри того, что вы анализируете, значение от установки не зависит.
...
Собственный `extends` анонимного класса не засчитывается в DIT охватывающего
именованного класса. `class Host { public function make() { return new class
extends Base {}; } }` даёт `Host.design.dit = 0`: цепочка родителей
принадлежит анонимному классу, у которого нет собственной декларационной
идентичности, а не `Host`. Это верно на любой глубине вложенности анонимного
класса.

Одно имя, объявленное более чем в одном файле, -- форма, которую даёт полифилл
под `class_exists()`, -- получает свою глубину на каждое объявление, поэтому две
находки по такому имени могут сообщать разные глубины: каждая измерена от того
родителя, которого наследует её собственное объявление. Родитель, названный в
`extends`, остаётся именем: когда его объявления расходятся в глубине, берётся
наибольшая -- и для глубины потомка, и для единственного значения, которое класс
несёт в `--format=metrics`, в HTML-дереве и в агрегатах. Два объявления одного
имени внутри одного файла измеряются как одно -- только последнее, -- и глубина
следует этому измерению, поэтому такие файлы сообщают ровно то же, что и
раньше.
