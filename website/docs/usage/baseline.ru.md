...
Baseline хранит принятый технический долг, чтобы существующий проект мог начать использовать Qualimetrix без требования немедленно исправить все текущие нарушения. Версия 10 — это **потолок по сообщаемой величине**, а не список игнорируемых хешей: существующая группа остаётся принятой только пока она не растёт и не ухудшается.
...
`baseline:migrate <baseline> [<paths>...]` обязателен для файла версии 5. Он создаёт свежий v10 capture, потому что v5 не хранила границу величины, и сообщает о записях v5, которые больше не срабатывают. `--force` позволяет создать свежий capture, даже если путь назначения не указывает на файл v5; используй его только когда замена этого файла намеренна.
