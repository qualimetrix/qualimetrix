...
Отсюда воспроизводимое расхождение. Проверено исполнением: `// @qmx-ignore-next-line
annotation.unused-directive` строкой выше протухшей директивы — `check` печатает
`annotation.unused-directive` на строке самой этой директивы (то есть считает её
ничего не погасившей), а `qmx directives` называет её `effective`. Права
команда: находку соседа она действительно погасила, что видно по тому же
прогону `check` — находка соседа не напечатана.

- **Цена:** два ответа продукта об одной директиве в одном прогоне, и
  докблок `DirectiveUsage` больше не может обещать «одно вычисление — один
  ответ» без оговорки.
- **Что закроет:** двухпроходный `stale()` (второй проход судит по
  `produced` + выход первого) — это меняет публикуемый канал, требует
  объявленной дельты гейта и рассуждения о неподвижной точке, поэтому это
  предмет П1, а не хвост команды. Сужать половину вердиктов под канал нельзя:
  так возвращается дефект, из-за которого любое подавление этого канала было
  инертным по построению.
