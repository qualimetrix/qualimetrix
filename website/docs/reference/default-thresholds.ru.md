...
**Code Duplication** обнаруживает дублированные блоки кода. Настраивается через `min_lines: 5` и `min_tokens: 70` -- блок короче `min_tokens` токенов игнорируется, как и каждая его копия, занимающая меньше `min_lines` строк. Дубликаты менее 50 строк выдают предупреждение; 50 строк и более -- ошибку.
