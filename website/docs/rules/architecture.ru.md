...
### Гейт неотнесённых объявлений { #unassigned-class }

`architecture.unassigned-class` отвечает на вопрос, который
...
По умолчанию выключен и настраивается на правиле, а не в секции `architecture:`:

```yaml
rules:
  architecture.layer-violation:
    unassigned_class: warn   # ignore (по умолчанию) | warn | error
```
...
`unassigned_class: ignore` или накройте объявления слоем.
...
| `unassigned_class` | `ignore`     | Гейт `architecture.unassigned-class` — счёт проанализированных class-like объявлений вне всех слоёв. Допустимо: `ignore`, `warn`, `error`. См. [Гейт неотнесённых объявлений](#unassigned-class). |
...
`--layer-violation-unassigned-class` задаёт гейт выше, — как и у других правил
архитектуры.
...
Чтобы подавить нарушение слоя, адресуйте точный канал: `@qmx-ignore architecture.layer-violation`. Более короткой формы нет — сопоставление по префиксу убрано, поэтому голое `@qmx-ignore architecture` — это ошибка, а не замена всей семьи. `architecture.*` тоже соблазнительна, но неверна: она заодно затронула бы не связанное с этим правило `architecture.circular-dependency`, а поскольку у `architecture.layer-violation` только один канал (он сам), `architecture.layer-violation.*` не совпадает ни с чем и падает ошибкой. «Все каналы политики слоёв» поэтому невыразимы по замыслу. Правило публикует семь каналов, но остальные шесть несут собственные имена правил (`architecture.coverage`, `architecture.unassigned-class`, `architecture.unreachable-layer`, `architecture.pending-layer-matched`, `architecture.potential-shadow`, `architecture.empty-template`), так что ни один селектор их не охватывает. Пять из этих шести суть ошибки конфигурации, которые никакое подавление не принимает; `architecture.unassigned-class` — обычный долг, но это сводный результат за прогон, отнесённый к проекту, поэтому инлайн-директива не дотягивается и до него: от него отказываются через `unassigned_class: ignore` или принимают в baseline.
...
**Исключение на уровне правила (`exclude_namespaces` / `exclude_paths`) тоже работает здесь.**
[Глобальный `exclude_namespaces`](../getting-started/configuration.ru.md#исключение-неймспейсов-exclude_namespaces)
...
Это работает, потому что фреймворк (`RuleOptionsFactory`) извлекает `exclude_namespaces` /
`exclude_paths` для любого имени правила безусловно, ещё до того, как конфигурацию увидит класс
Options самого правила. Явное указание `architecture.layer-violation` — однозначный, проверяемый
выбор: в отличие от общей записи `exclude_namespaces`, его нельзя прочитать как «просто исключить
