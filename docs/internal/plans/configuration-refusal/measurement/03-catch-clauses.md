...
## Числа «после», снятые приёмкой раунда

Сняты оркестратором последним действием, на переставшем меняться дереве `src/` и `bin/`,
теми же командами, что записаны в шапках. Пакеты их не снимали и не могли: число,
снятое на ещё меняющемся дереве, несопоставимо с «до».

| измерение                                                              | до  | после |
| ---------------------------------------------------------------------- | --- | ----- |
| статически перечислимые броски `InvalidArgumentException` вне носителя | 178 | 147   |
| `catch`-клаузы `src/` и `bin/`                                         | 76  | 87    |

Остаток убыл на 31 места. Он не обнулён, и раунд этого не обещал: `src/Infrastructure/Rule`
и `src/Infrastructure/Git` решением обзора остаются на общем `catch (InvalidArgumentException)
→ 3` и **считаются** в остатке, а не конвертируются.

Клауз стало больше, и это ожидаемо: раунд заводит клаузы, а не только снимает их.
Полный список расхождения с перечислением «до», по файлам:

| файл                                                | до  | после | чем объясняется                                              |
| --------------------------------------------------- | --- | ----- | ------------------------------------------------------------ |
| `Infrastructure/Console/Application.php`            | 0   | 4     | лестница, назначающая код явно; ею и отменяется 255          |
| `Console/Command/GraphExportCommand.php`            | 0   | 3     | собственная лестница команды с машинным форматом             |
| `Console/ScopeWarningChecker.php`                   | 3   | 6     | три глотающих клаузы сужены: носитель перебрасывается до них |
| `Console/Command/Debug/LayerAssignmentCommand.php`  | 1   | 3     | носитель плюс вторичный признак                              |
| `Console/Command/DirectivesCommand.php`             | 2   | 3     | носитель первой клаузой                                      |
| `Console/Command/BaselineRenameChannelsCommand.php` | 1   | 2     | отказ переноса заворачивается в носителя в самой команде     |
| `Console/Command/CheckCommand.php`                  | 7   | 4     | снятые клаузы удалённых типов                                |

**Довод о полноте класса 1 после раунда не ослаб, а усилился.** Каждая новая клауза либо
отдаёт носителя коду 3, либо перебрасывает его выше глотающей клаузы, либо — в случае
`Application` — назначает код явно и тем закрывает выход наружу, который и был кодом 255.
