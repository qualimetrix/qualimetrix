...
`architecture.potential-shadow` ловит тихий режим отказа declaration-order-сопоставления: когда класс матчится несколькими слоями, побеждает только первый, и более ранние слои могут тихо отбирать классы, которые пользователь рассчитывал отдать более позднему, более узкому слою. Это диагностика конфигурации (см. врезку в разделе [Режимы покрытия](#режимы-покрытия-coverage)): она валит прогон безусловно, как только срабатывает, и не настраивается, не бейзлайнится и не подавляется `@qmx-ignore`.
