...
final class ConfigurationPipelineIntegrationTest extends TestCase
{
    ...
    public function zeroConfigExperience_composerJsonExists_autoDiscoversPaths(): void
    {
        ...
        self::assertSame(['src', 'tests'], $resolved->paths);
        self::assertSame(['vendor', 'node_modules', '.git'], $resolved->pathExcludes);
    }
...
YAML;
        ...
        self::assertSame(['custom/'], $resolved->paths, 'CLI paths should override config file and composer.json');
...
YAML;
        ...
        self::assertSame(['lib/', 'app/'], $resolved->paths, 'Config file should override composer.json paths');
    }
...
YAML;
        ...
        // Assert
        self::assertSame(['src-custom/'], $resolved->paths);
    }
...
YAML;
        ...
        self::assertSame(['lib/'], $resolved->paths, 'paths from config file (overrides composer)');
        // Excludes merge across all stages: defaults + config file + CLI
        $excludes = $resolved->pathExcludes;
        self::assertContains('vendor', $excludes, 'excludes from defaults');
        self::assertContains('cache/', $excludes, 'excludes from config file');
        ...
        self::assertTrue($resolved->runtime->cacheEnabled, 'cache.enabled defaults to true');
    }
...
YAML;
        ...
        self::assertSame(
            $resolved->runtime->projectRoot->value() . '/custom-cache',
            $resolved->runtime->cacheDir->value(),
        );
    }
...
YAML;
        ...
        self::assertSame('/explicit/cache', $resolved->runtime->cacheDir->value());
    }
