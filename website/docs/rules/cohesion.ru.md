...
TCC и LCC в настоящее время доступны как **метрики** (видимы в выводе `--format=metrics`). Они не генерируют нарушения самостоятельно. Используйте их вместе с [LCOM](design.ru.md#lcom----недостаток-связности-методов) для полной картины связности класса.
