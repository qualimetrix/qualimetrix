...
| `TccLccVisitor`           | :96-101      | `TccLccClassData`        | `Class_`/`Trait_`; `Interface_`, `Enum_` отсеиваются раньше |
| `UnusedPrivateVisitor`    | :238-243     | `UnusedPrivateClassData` | набор `match` в `enterClassLike()`                          |
| `RfcVisitor`              | :132-137     | `ClassRfcData`           | `Class_` + `Interface_` (ветка ниже)                        |
...
потому что все восемь берут абсолютный `getStartFilePos()`. Это — источник
требования R3-инварианта в плане (считать до собственного фильтра) и
drift-guard-теста.
