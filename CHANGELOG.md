...
### Breaking
- The rule layer's `RuleLevel` is gone; `SymbolLevel` is now the project's one level vocabulary, and `HierarchicalRuleInterface`, `HierarchicalRuleOptionsInterface` and `Violation::$level` name it instead. No channel name, configuration key, report field or baseline entry changes. One behaviour does: a computed metric configured with `levels: []` emits nothing and is now treated as declaring no channel at all, so a baseline entry naming it goes inert instead of being compared.
