...
числе «ничего не совпало»), `1` — отказ по содержимому или baseline/карта
недоступны как файл, `2` — некорректное значение `--format`. Отказ сообщается в
выбранном формате: при `--format=json` это объект с ключом `error`.
...
попадают в счётчик отчёта. Попасть в счётчик не значит быть пропущенной: такая
запись всё равно называет канал, поэтому карта переименовывает её наравне с
остальными — оставить её на упразднённом имени было бы хуже, чем перенести.
Счётчик намеренно уже, чем то, что `check` называет inert: перенос не запускает
анализ, поэтому считает только то, что видно по самому документу, — запись не
объект, запись без читаемого `channel`, запись с испорченным `occurrence` или
`edge`, запись, уже делившая идентичность с другой, и субъект, хранящий свои
записи не JSON-массивом.
