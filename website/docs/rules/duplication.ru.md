...
!!! info "Deviation from original spec"
    ...
    токенов. Subject нарушения — проект; выбор primary-файла, строки, порядок
    обнаружения и порядок связанных копий влияют только на представление.
    Поэтому добавление лексически более ранней копии не меняет существующую
...
!!! warning "Инлайновый `@qmx-ignore` не подавляет этот канал"
    ...
    сопоставляются с первичным расположением находки — той копией, которую
...
!!! info "Все копии блока — одна находка"
    Все копии одного дублированного блока сообщаются вместе одной находкой, сколько бы их ни было, — верхнего предела на число копий нет. Сообщение называет число вхождений и до десяти других копий, а если их больше — добавляет `and N more`; связанные расположения находки перечисляют каждую копию. Если копии совпадают на разную длину, сообщается каждое наибольшее совпадающее множество: две копии, совпадающие на 40 строк, и третья, совпадающая с ними только на первых 10, дают одну находку на две 40-строчные копии и одну на все три по 10 строкам.
