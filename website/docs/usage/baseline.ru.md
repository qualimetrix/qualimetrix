...
Два флага `--include-*` определяют, что считается проектом, — пути, которые анализирует прогон без путей, и скоуп, по которому он оценивается, — поэтому baseline, снятый без флага, с которым потом запускается `check`, измеряет не тот набор. Они также принимают `--config=CONFIG`. Они **не** принимают `--suppress-path` и `--suppress-namespace`, потому что эти безопасные сужения `check` иначе сделали бы lifecycle-операции асимметричными. Они также не принимают `--no-suppression-annotations`: эта опция влияет только на отчёт и не может расширить измеряемый набор.
...
`--format=json` это тот же конверт `{error, exit_code, position}`, что и у любого
...
- **Новое имя не проверяется по каналам, объявленным этой сборкой.** Перенос
  ...
  релиза, объявляющего новое имя, `check` сообщает о перенесённой записи как о
  неприменимой. Это промежуточное состояние задумано именно таким.
- **Селекторы записей меняются.** Селектор — дайджест идентичности, в которую
  входит имя канала, поэтому сохранённый `baseline:cleanup --remove=<selector>`
  перестаёт адресовать перенесённую запись. Возьми селекторы заново из свежего
  вывода `baseline:cleanup`.
...
Переименование канала может сдвинуть запись среди соседей. Перенесённый файл
размещает все строки в том же каноническом порядке, в каком пишет сам продукт,
поэтому следующая команда, переписывающая файл, строк уже не двигает. Байты
каждой строки при этом остаются такими, как их написал файл, — именно это
позволяет перенести файл чужой сборки без переформатирования; строка, поля
которой расставлены вручную в необычном порядке, будет перерисована на месте —
но не сдвинута — при следующей перезаписи файла.
...
Строка `baseline:` говорит, сравнивает ли `check` эти два числа вообще:

| Вывод                                                                                                          | Значение                                                                                                                     |
| -------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `accepted 25; now 31`                                                                                          | Запись применяется: текущая группа сравнивается с принятым уровнем.                                                          |
| `accepted 24 (mode: suppress, accepted whatever is reported); now 32`                                          | Запись с `mode: suppress`: числа показаны, но группа принимается при любых значениях.                                        |
...
| ---------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `annotation.unresolved-directive`  | директива называет несуществующий канал (опечатка, имя правила там, где нужен канал, `X.*`, не совпавший ни с чем, или удалённая вычисляемая метрика) — **либо** она вообще не стала директивой: тег `@qmx-`, который инструмент не читает, или декларационная форма, написанная там, где ничего не измеряется |
...
Тег с опечаткой и `@qmx-ignore`, написанный над инструкцией или на свойстве, раньше тихо ничего не делали; теперь обе формы — ошибки `annotation.unresolved-directive`. См. раздел «Формы, которые директивой так и не становятся» на странице [Правила аннотаций](../rules/annotation.ru.md).
