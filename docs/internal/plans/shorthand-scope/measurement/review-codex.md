...
- **evidence**:
  ...
  ```
  План (01-contract.md) закрепляет DoD только для направления «нижний слой = shorthand, верхний = блок»; обратное направление нигде не оговорено.
...
- **evidence**:
  - `01-contract.md:60-62`: «A block that names `max_warning` and not `max_error` still takes the rule's own default for the half it did not name — the shorthand does not reach into a block to fill it.»
...
- **in_scope**: да
- **anchor**: контракт присутствия level-блока и его enablement по умолчанию (00-overview.md / 01-contract.md формулировка "block re-enables the level")
- **evidence**:
  ```php
...
- **in_scope**: да
- **anchor**: контракт M1 «outcome identical either way» (01-contract.md) vs фактический порядок «recognition → fromArray» в `RuleOptionsFactory::create()`
- **evidence**:
  ```php
