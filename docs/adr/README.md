...
- [0017 — Baseline Ceiling](0017-baseline-ceiling.md) — baselines cap the reported magnitude they accepted; amended 2026-09-24: the baseline commands accept the project-scope flags `--include-generated` and `--include-autoload-dev`, `baseline:migrate` is gone, and a duplicate block re-keys on its matched tokens rather than on its first copy.
...
- [0085 — A Copy of a Duplicate Block Is a Finding of Its Own](0085-a-copy-of-a-duplicate-block-is-a-finding-of-its-own.md) — `duplication.clone` reports one finding on each copy under the block's shared identity, so a baseline entry bounds the number of copies and a pasted copy is a breach in its own file.
