...
- **Цена:** пара свидетелей ловит расхождение путей (discovery, `exclude_*`,
  ...
  ровно тот класс, ради которого вторая мера заводилась. Докблок гейта это
  теперь называет прямо.
- **Что закроет:** вторая мера, устроенная структурно иначе, чем регулярка, —
  например, снятая тем же токенизатором, что и остальные перечисления захода.
...
- **Цена:** тот же fail-open по неузнанной форме, ради устранения которого
  переписывался страж.
- **Что закроет:** белый список измеренных значений, неизвестное — отказ шага.
  Туда же: `runEnumeration()` берёт три колонки строки TSV без проверки их числа.
...
Счёт кейсов написан «fourteen» в четырёх живых местах при шестнадцати каталогах
под `finding-gate/cases/`: `finding-gate/README.md:397`,
`scripts/finding-gate-controls/Controls.php:840`,
`scripts/finding-gate/RenameMaps.php:50`, `SelfTest.php:612`. Пятое место B1
переписал заодно — `TreeRun.php` держал этот счёт внутри объяснения, которое
перестало быть верным. Первая редакция этой записи называла семь мест и была
неверна: `Harness.php:39-40` и `Shell.php:324` считают не кейсы, а **контроли в
полёте** — их сегодня семнадцать, и это вторая, отдельная устарелость того же
класса; `ReportPayload.php:17` — измерение Ш5e3, предмет прошедшего времени и
правке не подлежит.
...
- **Цена:** та же, что у счёта поверхностей, и она уже реализовалась однажды.
  ...
  одно из названных мест не несёт исполняемой арифметики — цена пока только в
  чтении.
- **Что закроет:** те же два шага, что у счёта поверхностей, — перечисление
  скриптом и правка по таблице. `enumeration-surface-count.sh` этот счёт не
  ищет: его второй проход требует на строке слово `format`, `surface`, `формат`
  или `поверхность`, а
  предмет здесь другой — кейс и контроль. Скрипт об этом ограничении говорит
  сам, и закрытие начинается с его субъектных слов, а не с окна числительных.
