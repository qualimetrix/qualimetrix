...
верно на любой глубине и в любой секции:
...
Внутри `rules:` на этом равенство заканчивается. Сама запись остаётся: ключ
по-прежнему **написан**, просто написан без значения. Ключу, значение которого
только читают, это безразлично — `enabled: ~` выше равно молчанию, — но два
читателя спрашивают о **присутствии** ключа, а не о его значении, и для них
`~` молчанием не является:

- **Пороговый ключ выбирает плоскую форму иерархического правила.** У
  `complexity.ccn`, `complexity.cognitive` и `complexity.npath` это `threshold:`;
  у `coupling.cbo` — `threshold:`, `warning:` или `error:`; у
  `coupling.instability` — `threshold:`, `max_warning:` или `max_error:`.
  Написанный на верхнем уровне самого правила, такой ключ выбирает сокращённую
  форму, описанную выше, — и, как там, блоки `callable:` / `class:` /
  `namespace:`, написанные рядом, уже не читаются. `~` здесь ничем не отличается
  от числа:

    ```yaml
    rules:
      coupling.cbo:
        warning: ~       # НЕ то же самое, что не писать его
        class:
          warning: 0     # не будет прочитано: форму уже выбрал ключ выше
          error: 0
    ```

    Чтобы настраивать уровни, ключ нужно убрать: тогда правило ведёт себя так,
    как будто на его верхнем уровне не написано ничего.

- **`threshold:` и `warning:` / `error:` по-прежнему два режима.** Написанный
  рядом с любым из них `threshold: ~` приводит к отказу, а не к откату на
  градуированный режим:
...
**Элемент списка** — второе место, где это не действует, и по причине:
