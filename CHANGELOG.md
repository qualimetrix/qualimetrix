...
- `architecture.layers[].name` with capture variables now rejects `[` brackets at config-load time with an actionable hint suggesting `{var}` syntax. Previously brackets were silently treated as glob character classes, leading to semantic divergence.
