...
| **Structure**                                                        |                 |                  |                                                                                                                                                                                   |
| `lcom`                                                               | Structure       | Class            | LCOM4 — method cohesion (graph components; edges from shared properties and `$this->method()` calls; static methods excluded; stateless constant methods merged into one node)    |
...
| **Coupling**                                                         |                 |                  |                                                                                                                                                                                   |
| `ca`, `ce`, `cbo`, `instability`                                     | Coupling        | Class            | Afferent/Efferent Coupling, CBO (C&K), instability                                                                                                                                |
| `cbo_app`                                                            | Coupling        | Class            | Application-only CBO (excludes framework dependencies via `coupling.framework-namespaces` config)                                                                                 |
| `ce_framework`                                                       | Coupling        | Class            | Framework efferent coupling (outgoing deps to configured framework namespaces)                                                                                                    |
