...
!!! tip
    Метки уровня здоровья (Strong / Good / Acceptable / Weak / Critical) дают быструю оценку серьёзности. Но сравнивайте **числовые значения**, а не метки -- шесть проектов с меткой "Acceptable" могут иметь баллы от 45 до 68.
...
Вид пространства имён показывает оценки здоровья по нему, дочерние пространства и худшие классы внутри. Ищите пространства имён с оценкой ниже 50 (Weak или Critical).
...
**Не игнорируйте измерения оценки здоровья.** Класс с "Strong" по сложности, но "Critical" по связности требует другого подхода к рефакторингу, чем класс с "Critical" по сложности.

**Не сравнивайте значения ClassRank между проектами** разного размера. PageRank размывается с ростом графа.

**Не полагайтесь только на общую оценку здоровья.** Погружайтесь в оценки по измерениям. Проект с "Acceptable" общей оценкой может скрывать "Critical" по сложности за счёт "Strong" по типизации.
