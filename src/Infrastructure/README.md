...
- `RulesCommand` asks `RuleOptionSurface` what each rule accepts at each depth,
  so the listing and the option refusal answer from one declaration; it
  additionally reads `RuleChannelRegistryInterface` and
