...
- **П3 и П4 не начаты и не подготовлены исподволь:** словарь имён не менялся,
  в `finding-gate/maps/` ни одной строки не объявлено.
...
- **`architecture.coverage` как ключ конфигурации — решён, исполняется в П4.**
...
## Х12П4 (2026-09-08) — неизвестный ключ уровня в опциях правила отбрасывается молча

- **Что измерено.** На дереве после переименования:

      rules: {complexity.ccn: {method:   {warning: 1, error: 2}}}  -> 0 находок, ни слова
      rules: {complexity.ccn: {callable: {warning: 1, error: 2}}}  -> 1 находка

  Неизвестный **ключ уровня** внутри опций правила не даёт ни отказа, ни
  предупреждения: порог просто не применяется. Именно поэтому `README.md` и
  `qmx.yaml.example` учили форме `method:`, которая ничего не делает, и это
  прожило до ревью Х12.

- **Почему это не тот класс, что уже закрыт.** Ш5e3 (выше в этом файле) закрыл
  соседний: «неизвестный ключ внутри правила давал предупреждение, а не отказ»,
  и каждому снятому написанию завели именованный отказ. Закрыто **поимённо**,
  для написаний того шага. Позиция «ключ уровня» осталась, и она тише того, что
  чинил Ш5e3: там было предупреждение, здесь нет ничего.

- **Почему не в Х12.** Заход переименовывает опубликованные имена; это
  продуктовое поведение, предшествующее ему, и лечение — отказ на неизвестный
  ключ уровня — меняет контракт конфигурации, а не словарь. Документация
  приведена к рабочей форме (`callable:`) тем же заходом.

- **Что закроет:** неизвестный ключ уровня внутри опций правила отвергается так
  же громко, как неизвестный владелец опции (exit 3), с перечислением
  допустимых уровней этого правила; регрессионный тест на каждую позицию, где
  ключ может быть не узнан.
