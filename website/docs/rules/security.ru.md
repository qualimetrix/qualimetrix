...
- идентификаторы через точку вроде `auth.password.reset`, `App.Models.User` или `auth.password-reset` (ключи переводов, конфигурации или каналов: каждый сегмент — либо идентификатор кода, то есть буква или подчёркивание, за которыми идут буквы, цифры или подчёркивания, либо слова из строчных букв, соединённые дефисами). Сегмент с дефисом, в котором есть заглавная буква или цифра, сообщается, поэтому `Summer-2024.Pass` и `sk-live.abc123-def456` ключами не считаются; JWT (`eyJ...`) — тоже. Значение только из строчных слов вроде `sk-live-abc.def` имеет форму ключа и пропускается;
...
Один запрос — одно нарушение, сколько бы суперглобальных переменных он ни читал. Если функция запроса, вызов `sprintf()` или конкатенация уже сообщили суперглобальную переменную, запросы, собранные внутри них, сообщаются повторно только за чтение, до которого внешний запрос не дошёл: `mysqli_query($conn, "SELECT * FROM users WHERE name = '{$_POST['name']}'")` — одно нарушение, а не одно на вызов и ещё одно на интерполированную строку. Запрос за любым другим вызовом по-прежнему сообщается сам: в `mysqli_query($conn, trim("SELECT * FROM users WHERE id = " . $_GET['id']))` сообщается конкатенация, а в `"SELECT * FROM t WHERE a = " . $_GET['a'] . " AND b IN (" . implode(',', ["SELECT id FROM u WHERE n = '{$_POST['n']}'"]) . ")"` подзапрос сообщается за `$_POST` рядом с `$_GET` внешнего запроса.
