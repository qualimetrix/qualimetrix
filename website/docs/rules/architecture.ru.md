...
Коды выхода соответствуют стандартному соглашению: `0` для любого информационного результата (включая "класс не соответствует ни одному объявленному слою"), `3` для отказа — некорректного ввода (пустой или некорректный FQN) или ошибки загрузки конфигурации, — и `1` только для дефекта, который ввод не мог вызвать.
