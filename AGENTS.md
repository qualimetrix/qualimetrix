...
| `scripts/generate-modular-architecture-test-inventory.php` — the scan-scope literal in `inventoryScanScope()` (the `git ls-files` path list feeding discovery). Loud since `assertEveryTestDirectoryIsClaimed()` began reading it: a test-shaped directory git carries and the scope misses is refused by name. Still silent for a root whose test directory is not test-shaped — its own row, two below | loudly       |
| `scripts/generate-modular-architecture-test-inventory.php` — `NON_ROOT_TEST_DIRECTORIES`. Not an address a root visits: it is where a test-shaped directory that is **not** a root is excused. `assertEveryTestDirectoryIsClaimed()` refuses an entry that is stale, redundant or reasonless, and refuses a test-shaped directory the scan does not reach                                                | loudly       |
