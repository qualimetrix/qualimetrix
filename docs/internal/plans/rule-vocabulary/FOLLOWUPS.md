...
Бар рисуется в секции над потоком ошибок, а `DiagnosticOutput::stream()`, фабрика
логгера и `CheckCommand::writeWarning()` пишут в `getErrorOutput()` напрямую,
мимо любых секций. Предупреждение во время прогона поэтому может разорвать кадр
бара. Фолбэков потока диагностики при этом три разных: потерять, `NullOutput`,
тот же поток.

- **Цена:** до Х3-G столкновения не было — бар жил в stdout. Теперь оно
  возможно, и это принято сознательно, а не пропущено.
- **Что закроет:** один владелец потока диагностики — тот же массив секций для
  всех четырёх швов и один фолбэк вместо трёх. Это отдельный предмет: правка
  трогает четыре чужих шва и не помещается в пакет про поток прогресса.
...
`Qualimetrix\Infrastructure\Console\Command` из `health.cohesion` снимает
канал со всего корня команд, а не только с семи классов, стоявших на 40.
`CheckCommand` стоит ровно на пороге 50.0 — в нуле от восьмой записи ратчета, —
и после исключения не станет ни кандидатом в ратчет, ни наблюдаемым по этому
каналу вовсе.

- **Цена:** когезия срабатывала на этом корне раньше всех размерных каналов —
  шесть методов при `lcom=2` против двадцати у `size.method-count`. Распухание
  команды теперь ловится позже: `size.method-count` (20/20) и `cohesion.lcom`
  (5/5) на корне активны, но `BaselineExplainCommand` и `LayerAssignmentCommand`
  с их одиннадцатью методами до этих порогов ещё далеко. Раннее предупреждение
  было побочным эффектом канала, который мис-моделирует дизайн команды, и
  уходит вместе с ним.
- **Что закроет:** осознанно выбранный порог у канала про **размер** команды —
  `size.method-count` для этого корня, — а не возврат когезии. Если распухание
  надо ловить раньше двадцати методов, это отдельное решение с измеренной
  ценой, а не восстановление случайного сигнала.

### Подробный режим печатает лог в тот же поток, где рисуется бар

Найдено ревью исполнения Х3-G. README команд обещал, что `-v`/`-vv`/`-vvv`
выключают прогресс-бар («detailed logging is shown instead»); проверки verbosity
выше `QUIET` в коде нет и не было — измерено исполнением под псевдотерминалом:
при `-vvv` бар рисуется, три кадра. Ложный буллет удалён.

- **Цена:** после переезда бара в поток ошибок именно `-vvv` — режим, где бар и
  подробный лог гарантированно делят один поток и перетирают друг друга.
  Кадр секции затирает вклинившуюся строку лога; измерено, что так было и до
  переезда, то есть пакет G этого не ухудшил, но и не тронул.
- **Что закроет:** решение о том, кто уступает. Либо бар гасится при verbosity
  выше `NORMAL` (тогда обещание README становится верным), либо диагностика
  проводится через тот же массив секций, что и бар. Первое дешевле и честнее для
  режима, который просят ради чтения лога глазами.
...
Найдено пакетом Х3-H исполнением: шаг, который намеренно убирает одну находку,
доказать гейтом нечем. Измерено тремя прогонами против `38ad58e9` с корпусной
фикстурой, порождающей эту находку:
...
Второй исход и есть механизм: исчезнувшая запись уносит с собой все поля кортежа
эквивалентности, а объявленная дельта вправе двигать поле только внутри записи,
которую объясняет объявленный `split`. Перестановка коммитов не помогает —
фикстура отдельным коммитом даёт тривиальный GREEN (один продукт с обеих сторон).

- **Цена:** любой шаг, меняющий состав находок намеренно, доказывается только
  «всё остальное не сдвинулось» плюс тестами; сам предмет шага гейтом не
  свидетельствуется. Пока такой шаг один, но он не последний: запрет подавлять
  поздний канал упрётся в ту же стену.
- **Что закроет:** форма объявления «намеренная дельта множества находок»,
  ключуемая кейсом и точными множествами идентичностей исчезающих и
  появляющихся записей. Это правка самого компаратора, поэтому за ней обязателен
  повторный `composer gate:controls`: гейт, доказавший себя до переписывания,
  ничего не говорит о переписанном.
- **Х4 (2026-09-03): предсказание сбылось, и счёт уже конкретный.** Заход,
  запрещающий подавлять `annotation.unused-directive`, корпусную фикстуру не
  сажал — решением владельца. Поэтому `composer gate -- --reference=6f48b2a0` на
  нём зелёный и о предмете шага не свидетельствует ничего: он говорит ровно
  «постороннее не сдвинулось», и так и записан в отчёте. Первый конкретный счёт
  за отложенное решение: **правка подсказок «did you mean» снята с захода именно
  потому, что её нечем объявить** — она двигает опубликованное `message`
  корпусного кейса `annotations`, а `message` входит в кортеж эквивалентности.
  Это не «шаг доказан слабее», а «шаг вообще не поставлен».

### Символьная форма адресует канал, который она не может погасить никогда

Найдено ревью плана Х3-H и подтверждено исполнением.
...
- **Цена:** автор получает совет снять аннотацию, которая не могла сработать
  ни при каких условиях, и никакой диагностики о том, что форма выбрана не та.
- **Что закроет:** запрет подавлять этот канал инлайн (тогда форма отвергается
  внятно) либо отдельная диагностика «эта форма к этому каналу неприменима» —
  предмет, соседний с `annotation.unresolved-directive`.
- **Закрыто заходом Х4, пакет 2 (2026-09-03).** Выбрана первая развилка: форма
...
протухшего. Измерено на форме с `@qmx-ignore-file annotation.*`: `check` не
печатает про канал ничего (обе жалобы съедены селектором), а `directives` в том
же прогоне говорит `1 effective, 1 inert`.

- **Цена:** «один ответ о директиве» достижимо только для одного из двух
  источников расхождения; второй — про то, что публикация фильтруется позже
  суждения.
- **Что закроет:** решение о том, судит ли аудит до фильтра публикации или
  после. Сегодня он судит до, и это, вероятно, верно (аудит отвечает про
  директиву, а не про отчёт) — но обещание докблока надо сузить до этого.
- **Сужено заходом Х4, пакет 2 (2026-09-03).** Именно тот экземпляр, на котором
  расхождение было измерено (`@qmx-ignore-file annotation.*`), снят: эта форма
  теперь отвергается, потому что её разворачивание достаёт до забаненного
  канала. Порядок стадий не тронут, и запись остаётся открытой — фильтр
  публикации по-прежнему стоит после конвейера, а аудит по-прежнему судит до
  него. Ушёл наблюдаемый пример, а не причина.

## Х4 (2026-09-03) — запрет подавлять `annotation.unused-directive`

### Подсказки «did you mean» советуют канал, который нельзя погасить

`DirectiveNameHints` подбирает ближайшее имя к опечатке и может предложить
`annotation.unused-directive`. Автор напишет предложенное — и получит отказ:
этот канал не адресуется ни одной формой. Подсказка ведёт в стену, которую сама
же не видит.

- **Цена:** два сообщения об одной опечатке вместо одного, и второе отменяет
  первое. Отказ объясняет, почему так, поэтому автор не заперт — но путь длиннее
  на один прогон.
- **Почему не исправлено на Х4 — измерено, а не оценено:** правка двигает
  опубликованное `message` корпусного кейса `annotations`, а `message` входит в
  кортеж эквивалентности гейта. Объявить это движение гейту нечем (см. запись
  «Гейт не умеет объявить намеренное изменение множества находок»), поэтому шаг
  снят с захода целиком, а не сделан «без доказательства».
- **Что закроет:** исключение забаненного канала из множества кандидатов
  подсказки — вместе с формой объявления дельты у гейта, иначе шаг снова нечем
  провести.
...
Измерено ревью исполнения пакета 2 на фикстуре и воспроизведено оркестратором:
`rules.annotation.directive.exclude_paths` и
`rules.annotation.directive.exclude_namespaces` находку
`annotation.unused-directive` **не гасят** — леджер исключений закрывается
вместе с исполнением правил, а этот канал собирается после него. Верхнеуровневый
`exclude_namespaces` не гасит её по другой причине: субъект находки — файл, в
котором написана аннотация, и неймспейса он не несёт. Работают только
верхнеуровневый `exclude_paths` (файл выпадает целиком), ратчет и git-скоуп.

- **К запрету отношения не имеет:** так было и до него. Запись заведена потому,
  что первая редакция документации обещала `exclude_namespaces` как замену
  снятому подавлению, и обещание было неверным.
- **Цена:** единственный способ выключить канал «точечно» — выключить правило
  `annotation.directive`, а вместе с ним гаснут три диагностики-ошибки
  конфигурации.
- **Что закроет:** решение о том, где собирается этот канал, — либо он
  порождается внутри исполнения правила и проходит леджер, либо у поздних
  каналов заводится собственная стадия исключений. Оба варианта трогают порядок
  публикации, то есть предмет соседней открытой записи.

### `exclude_namespace_channels` не может назвать канал с дефисом

Найдено ревью плана Х4 измерением. Ключ секции `rules` камелкейсится
нормализацией, поэтому `exclude_namespace_channels` с именем канала, содержащим
дефис, отвергается на старте прогона. Почти все каналы проекта пишутся в kebab,
то есть опция применима к меньшинству имён, а не к исключениям.

- **Цена:** документированная опция не работает для большинства каналов, и
  отказ приходит как ошибка конфигурации, а не как «такого канала нет» — автор
  не понимает, что дело в написании ключа.
- **К запрету отношения не имеет:** дефект самостоятельный и старше его. На Х4
  он важен ровно тем, что был **основанием** первой редакции плана («сужение
  `expand()` сломает этот ключ»), а измерение показало: ключ и так не работает,
  и настоящее основание другое — молчаливое сужение групповой формы `annotation.*`.
- **Что закроет:** политика нормализации секции `rules`, сохраняющая написание
  ключей `exclude_*_channels` как имён каналов, а не как ключей конфигурации
  (ADR 0009 — модель, куда такое правило вписывается).

### `Outcome::asDeclared()` сверяет объявленное подмножеством, а не равенством

Найдено ревью исполнения пакета 2 и **не закрыто**.
`scripts/directive-audit-controls/Outcome.php` считает пробник «as declared»,
когда все объявленные им кейсы покраснели (`missing === []`) и покраснели не
все кейсы прогона. Кейсы, которые пробник краснит **сверх** объявленных, не
проверяются ничем, кроме верхней границы «не всё сразу».

- **Цена:** расползание пробника невидимо даже при честном объявлении. Пакет 2
  напоролся на соседний экземпляр того же класса — пробник объявлял имя метода,
  а имя метода считается покрасневшим от любого из двенадцати его датасетов, —
  и это чинилось поштучным измерением, а не харнессом.
- **Почему не правка этого захода:** равенство вместо подмножества — общая
  семантика харнесса, оно затрагивает все пробники сразу и требует
  перепроверки каждого объявления. Пакет 3 записи, менять код не вправе.
- **Что закроет:** `asDeclared()` требует равенства множеств, а пробник,
  которому нужно покраснить больше, объявляет это явно. За правкой обязателен
  полный `composer directives:controls` с поштучным снятием — харнесс,
  доказавший себя до смены семантики, о новой не говорит ничего.
