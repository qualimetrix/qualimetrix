...
Команда отказывает, оставляя файл побайтово неизменным, если: файл не версии
13; конверт не читается как документ baseline; записи субъекта не являются
JSON-массивом; две строки переименовывают одно имя; две строки дают одно имя;
обе стороны строки совпадают; цель одной строки переименовывается другой; либо
перенос дал бы двум записям одного субъекта одну идентичность. Объявленное
переименование, которое ничего не задело в этом файле, сообщается, а не
отвергается. Коды возврата: `0` — перенос выполнен (в том числе «ничего не
совпало»), `1` — отказ по содержимому или baseline/карта недоступны как файл,
`2` — некорректное значение `--format`.
...
Записи, которые эта сборка прочитать не может, переносятся без изменений, а не
удаляются, и попадают в счётчик отчёта. Этот счётчик намеренно уже, чем то, что
`check` называет inert: перенос не запускает анализ, поэтому считает только то,
что видно по самому документу, — запись не объект, запись без читаемого
`channel`, запись с испорченным `occurrence` или `edge` и запись, уже делившая
идентичность с другой.

Переименование канала может сдвинуть запись среди соседей. Перенесённый файл
записывается в том же каноническом порядке, в каком пишет сам продукт, поэтому
следующая команда, переписывающая файл, строк уже не двигает.
