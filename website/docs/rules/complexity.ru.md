...
Qualimetrix использует расширенный вариант цикломатической сложности, иногда называемый **CCN2+**: стандартные точки принятия решений (if, elseif, while, for, foreach, case, catch, &&, ||, ?:) плюс операторы PHP, перечисленные в отклонении ниже.

!!! info "Отклонение от оригинальной спецификации"
    У метрики Маккейба нет аналога этих операторов PHP. Qualimetrix считает каждый из них точкой принятия решения:

    - `??` (оператор null coalescing) и `??=` (присваивание с null coalescing) — +1
    - `?->` (nullsafe-вызов метода) — +1
    - `?->` (nullsafe-обращение к свойству) — +1
    - `xor` (логический оператор XOR) — +1, как и другие логические операторы, хотя он не вычисляется по короткой схеме

    `??`, `??=` и `?->` скрывают ветвление: `$a ?? $b` эквивалентно `$a !== null ? $a : $b` — точка принятия решения, которую легко упустить из виду.
...
- **Разрывы линейного потока добавляют баллы.** `goto` и `break`/`continue` с номером уровня (`break 2`) стоят баллов, потому что нарушают поток чтения.
...
Qualimetrix реализует спецификацию SonarSource
([Cognitive Complexity, версия 1.7](https://www.sonarsource.com/docs/CognitiveComplexity.pdf), приложение B):

- **Инкременты (+1):** `if`, `elseif`, `else`, тернарный `?:`, `switch`, `for`, `foreach`, `while`, `do-while`, `catch`, `goto`, `break`/`continue` с номером уровня, каждая последовательность одинаковых логических операторов и метод, вызывающий сам себя (+1 один раз на метод, сколько бы рекурсивных вызовов в нём ни было).
- **Последовательности логических операторов** читаются в порядке записи, как в спецификации: каждый оператор, отличный от предыдущего, начинает новую последовательность. `$a && $b && $c` — одна, `$a && $b || $c` — две, `$a && ($b || $c) && $d` — три. `&&` и `and` — один оператор, как и `||` и `or`. Всё остальное между двумя операторами — `!`, вызов, присваивание — завершает выражение, поэтому `$a && !($b && $c)` — две последовательности.
- **`else if`**, записанный двумя словами, — это `else if` спецификации: +1 без надбавки за вложенность, так же как `elseif`. `if` внутри `else { ... }` с фигурными скобками — вложенный `if`.
- **Надбавка за вложенность:** `if`, тернарный оператор, `switch`, циклы и `catch` добавляют ещё текущий уровень вложенности. `elseif`, `else if`, `else`, `goto` и переходы с номером — нет.
- **Уровень вложенности:** растёт внутри тел `if`, `elseif`, `else`, ветвей тернарного оператора, веток `switch`, циклов, `catch` и лямбд. Спецификация не говорит, где находится условие; Qualimetrix считает условие или проверяемое выражение не вложенным в структуру, которой оно управляет: тернарный оператор в условии `if (...)` добавляет +1, а в теле `if` — +2.
- **`match`** — switch-выражение PHP, оценивается как `switch` (+1 плюс уровень вложенности, ветви вложены на один уровень): спецификация засчитывает собственное написание перечисленного ключевого слова в языке.
- **Замыкания и стрелочные функции** ничего не добавляют вмещающему методу и поднимают уровень вложенности своего тела на один. Как и все остальные метрики callable, Qualimetrix измеряет каждое замыкание как отдельную единицу (см. отклонение ниже).
- **Именованные функции и методы анонимных классов**, объявленные внутри метода, измеряются как отдельные единицы с уровня вложенности 0 (см. отклонение ниже).
- **Операторы null coalescing ничего не добавляют:** `??`, `??=` и `?->` дают 0, как предписывает спецификация (раздел «Ignore shorthand», страница 6): они сокращают проверку на null до одного выражения, а не прерывают линейный поток. Цикломатическая сложность и NPath по-прежнему учитывают `??` и `??=`.

```php
public function activeNames(array $users): array  // когнитивная сложность: 0
{
    return array_map(                         // +0 (сама стрелочная функция)
        fn (User $u) => $u->isActive()        // стрелочная функция: +2 (тернарный оператор, вложенность 1)
            ? $u->name()
            : 'inactive',
        $users,
    );
...
!!! info "Отклонение от оригинальной спецификации"
    - **Замыкания и стрелочные функции внутри метода:** спецификация добавляет тело лямбды
      к вмещающему методу. Qualimetrix оставляет лямбду отдельной единицей, как это делают
      его цикломатическая, NPath-метрика и индекс сопровождаемости, но продолжает уровень
      вложенности с того места, где лямбда написана. Поэтому оценка метода по спецификации
      равна значению метода плюс значения лямбд внутри него; в примере выше 0 + 2 = 2.
      Находка о сложном замыкании выдаётся на само замыкание.
    - **Именованные функции и методы анонимных классов внутри метода:** спецификация
      вкладывает их на один уровень внутрь вмещающей структуры и добавляет их тело к методу.
      Qualimetrix измеряет каждую такую функцию как отдельную единицу с уровня вложенности 0.
    - **Рекурсия:** обнаруживается только прямой вызов самого себя: `$this->method()` или
      `$this?->method()`, `self::`, `static::` или собственное имя класса, либо функция,
      вызывающая себя по короткому, полностью квалифицированному или относительному
      (`namespace\`) имени, в любом регистре. Спецификация засчитывает и каждый метод
      косвенного цикла (A вызывает B, B вызывает A); Qualimetrix — нет и не отслеживает
      вызов через переменную или callable.
...
- **Расширения для PHP:** `match`, `foreach`, `??` и `?->` обрабатываются как конструкции, порождающие пути. Слоты выражений в `for`, `foreach`, `switch` и `echo` обходятся через прозрачные AST-обёртки.
...
Каждая ветвь вносит как минимум один путь, поэтому простой `match` совпадает с
эквивалентным `switch`. Nullsafe-вызов или доступ добавляет свой условный путь, а
вложенные тернарные и логические операторы не теряются в обёртках. Некоторые другие
инструменты (в частности pdepend) используют мультипликативный подход для `match`, который может
давать экстремальные значения. Значения этих конструкций меняются относительно прежних
версий Qualimetrix; пороги и baseline могут потребовать перекалибровки.

!!! info "Отклонение от оригинальной спецификации"
    Обработка `try/catch` в NPath — осознанное расширение qmx. Qualimetrix считает
    `(NPath(try) + Σ NPath(catch) + 1) × NPath(finally)`: `+1` — это аддитивный член
    в стиле PMD для пути без исключения. Поэтому голый `try{}catch{}` даёт NPath 3,
    тогда как оригинальная схема Немеха (1988) — которая появилась до исключений —
    подразумевала бы 2. Это отличается от phpmd/pdepend и сделано намеренно.
...
WMC вычисляется как сумма [цикломатической сложности](#цикломатическая-сложность) всех методов класса. Агрегатор формирует эту метрику уровня класса, а правило Complexity проверяет её и может исключать data-классы. Поскольку Qualimetrix использует вариант CCN2+ (который учитывает `??` и `?->` как точки принятия решений), значения WMC будут соответственно выше, чем у других инструментов.
