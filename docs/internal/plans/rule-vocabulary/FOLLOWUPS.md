...
- **Закрыто пакетом Х3-F (2026-09-03).** Контроль получил цель, конфигурацию и
  пол разнородности; вторым прогоном идёт фикстура
  `tests/Analysis/Policy/Inline/Fixtures/NarrowControl` со всеми четырьмя
  вердиктами и всеми четырьмя причинами. Запись при этом ошибалась в том, чем
  именно разнородность поможет: мутация «сужать всегда не тем правилом»
  сравнением вердиктов **не ловится и на разнородной популяции** — почему,
  сказано отдельной записью ниже. Что разнородность действительно покупает,
  измерено на другой мутации: сведение опорного отпечатка к полному прогону
  (`reference()` всегда возвращает `$baseline`) переворачивает четыре вердикта
  из восьми на фикстуре — код 1 — и ноль из сорока трёх на `src/`, где прогон
  остаётся зелёным. Это ровно тот класс «всё стало `Effective`», ради которого
  запись заведена.
...
- **Закрыто пакетом Х3-F (2026-09-03).** `--target` и `--config` — обязательные
  ...
  держит его вне перечисления по `src/` — по содержанию сайта, а не по пути,
  так что перенос фикстуры внутрь `src/` его краснит (проверено снятием).
...
### Мутация «сужать не тем правилом» до сравнения вердиктов не доходит вовсе

Запись П5 предполагала, что разнородная популяция даст сравнению вердиктов
увидеть эту мутацию. Измерено на фикстуре: не даёт, и не по слабости популяции,
а по построению. `narrowedTo()`, возвращающий чужое имя, ловит третий контроль
аудита — `assertNarrowingChangedNothing()`: правило, исполненное в одиночку,
обязано воспроизвести то, что оно произвело в полном прогоне. Он срабатывает,
как только адресуемое правило хоть что-то произвело в базовом прогоне, а
`Overrun` по определению требует, чтобы находка сработала. Значит **любая**
популяция, содержащая `Overrun`, роняет узкий свип в исключение раньше, чем
появится хоть один вердикт: контроль краснеет кодом 3 «прогон невалиден», а не
кодом 1.

- **Цена:** у сравнения вердиктов нет пробы, состоящей из этой мутации.
  Разнородность закрывает соседний класс дефектов (см. закрытую запись П5), а
  этот класс закрыт третьим контролем продукта — то есть проверяется юнит-тестом
  на самом контроле, а не прогоном обоих свипов.
- **Что закроет:** проба, оставляющая узкий прогон самосогласованным и всё же
  меняющая вердикт, — например, мутация в сравнении отпечатков, а не в выборе
  продюсера. Пока такой пробы нет, «сравнение вердиктов кусается» держится на
  мутации `reference()`, которая измерена и красит только на фикстуре.

### Перечисление мест, дающих непонятный код возврата, было короче самих мест

План Х3-F называл «десять мест», бросающих голое исключение. Их
одиннадцать плюс одно, отвечающее кодом 7: перечисление писалось по описаниям
исходов, а не по точкам `throw`, и потеряло проверку «отчёт сообщает о другом
свипе» и «корень репозитория неразрешим». Тот же класс дефекта, что уже
записан в памяти захода: перечисление по описанию не видит того, чего описание
не назвало.

- **Цена:** DoD, требующий «перевести десять мест», был бы выполнен при двух
  непереведённых. Здесь это ничего не стоило — все точки уже шли через один
  `catch`, — но в шаге, где перевод точечный, стоило бы.
- **Что закроет:** перечислять точки `throw` командой по файлу, а не исходы по
  памяти.

### Составной прогон контроля возвращает код первой неудачи, а не худший из двух

`composer directives:narrow-control` — две последовательные команды composer:
`src/` и фикстура. Composer прекращает список на первой ненулевой, поэтому код
цели — код первого упавшего прогона, а не максимум по двум, и второй прогон при
падении первого не исполняется вовсе.

- **Цена:** при сломанном `src/` разнородный прогон молчит, и наоборот; чтобы
  увидеть оба, их надо запустить по отдельности.
- **Что закроет:** либо оболочка, исполняющая обе цели и сводящая коды, либо
  явный отказ от «худшего из двух» в пользу «первого». Второе — то, что есть
  сейчас; запись существует, чтобы это не читалось как первое.
