...
Показать, что на самом деле делает каждая инлайновая директива `@qmx-ignore` и `@qmx-threshold` в анализируемом дереве. Подавление оценивается по тому, что оно погасило; пороговая директива — снятием и повторным исполнением правил поверх измерений того же прогона, что стоит одного исполнения правил на директиву.
...
Коды возврата: `0` — инертных нет, `2` — найдена хотя бы одна инертная директива с наблюдаемой границей, `3` — негодный ввод или ошибка конфигурации, включая охват, в котором не нашлось ни одного PHP-файла, `4` — прогон не разобрал часть дерева, `1` — сама команда неожиданно упала.
...
!!! info "Судим по произведённому правилами, а не по отчёту"
    ...
    `exclude_paths`, `exclude_namespaces` и `exclude_namespace_channels` подавляют **публикацию**, а не измерение. Директива, сдвинувшая находку внутри исключённого неймспейса, всё равно что-то сделала, поэтому аудит задаёт вопрос по всем находкам, произведённым правилами, — включая `annotation.unused-directive`, который прогон собирает уже после исполнения правил.

    Единственное, что подавлению не засчитывается, — гашение ошибки конфигурации (`annotation.unresolved-directive` и два соседних канала). Эти каналы выведены из-под аннотационного подавления по построению, а не настройкой, поэтому директива, адресующая любой из них, отчитывается инертной, как бы она ни была написана.
