...
`>(gzip > report.json.gz)`. `/dev/stdout`, `/dev/stderr`, `/dev/fd/N` и
`/proc/self/fd/N` пишутся через сам поток — терминал, канал или файл. Имя, под
которым ещё ничего нет, создаётся: отчёт пишется рядом и переименовывается в
него, поэтому неудавшаяся запись не оставляет файла. Запись, оборвавшаяся
посреди существующего файла, оставляет его записанным частично.

Цель, в которую нельзя записать, отклоняется с кодом 3 до начала анализа:
каталог или имя, оканчивающееся на `/` (или символическая ссылка на такое имя),
существующий файл, недоступный для записи, новое имя в каталоге, которого нет
или который недоступен для записи, или дескриптор, который процесс не держит
открытым. Запись, не удавшаяся уже после прогона, тоже завершает его с кодом 3.
...
для записи существующий файл и новое имя в каталоге, которого нет или который
недоступен для записи, отклоняются с кодом 3 до чтения первого файла;
