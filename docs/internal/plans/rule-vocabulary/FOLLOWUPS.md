...
- **Цена:** ложный `Inert` на коде возврата `2`, то есть красный CI с неверным
  требованием к автору.
- **Что закроет:** `RuleOptionsRegistry::isRuleDisabledByOptions()` обязан
  читать ту же поуровневую конфигурацию, что и исполнение. Проверяется
  директивой на правиле, выключенном поуровнево: сегодня `Inert`, должно быть
  `Unmeasured`.
