...
под `class_exists()`, -- получает свою глубину на каждое объявление, и находка
сообщает того родителя, которого объявление действительно наследует. Родитель,
названный в `extends`, остаётся именем: когда его объявления расходятся в
глубине, берётся наибольшая -- и для глубины потомка, и для единственного
значения, которое класс несёт в `--format=metrics`, в HTML-дереве и в
агрегатах. Два объявления одного имени внутри одного файла не различаются --
измеряется только последнее.
