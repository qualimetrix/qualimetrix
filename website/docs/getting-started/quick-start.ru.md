...
Команда записывает `.git/hooks/pre-commit`, указывая в нём путь к тому
исполняемому файлу, которым её запустили. Файл ваш: его можно править, хук
продолжит работать, а переустановка с `--force` сохранит прежнюю версию рядом.

Если хук уже есть, `hook:install` откажется его трогать без `--force`, а с
`--force` сделает резервную копию.

!!! warning "Обновление с 0.27.0 и раньше"

    Прежние версии ставили хук символической ссылкой на скрипт, который больше
    не поставляется, — ссылка повисла, и git не запускает ничего. Выполните
    `vendor/bin/qmx hook:install --force`. То же самое скажет `hook:status`.
...
services:
  qmx:
    # Образ, собранный командой `docker build -t qmx .` выше, в checkout'е
    # Qualimetrix. В реестр образ пока не публикуется.
    image: qmx:latest
    volumes:
      - .:/app:ro
