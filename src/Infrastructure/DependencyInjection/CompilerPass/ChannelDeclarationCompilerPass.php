 ...
 * {@see assertJudgedMetricsAreDeclarable()}, which also names the six channels
 * the check says nothing about. It lives in this pass for the same reason the
 * shape checks do: {@see ChannelDeclaration} would otherwise have to import
 * {@see MetricName} across a capability boundary to assert it.
 *
...
final class ChannelDeclarationCompilerPass implements CompilerPassInterface
{
    ...
    private function collectRuleChannels(
        ...
        foreach (ChannelDeclarationReader::read($class) as $key => $declaration) {
            ...
            self::assertJudgedMetricsAreDeclarable($key, $class, $producerRuleName, $shapeByRule[$producerRuleName], $declaration);
    ...
    private function collectValidatorChannels(
        ...
        foreach ($validatorDeclarations as $key => $declaration) {
            ...
            self::assertJudgedMetricsAreDeclarable($key, $class, $producerRuleName, $shape, $declaration);
     ...
     * edge between two capabilities. This pass already composes both sides.
     *
     ...
     * @param class-string $class
     */
    private static function assertJudgedMetricsAreDeclarable(
        string $key,
        string $class,
