...
`architecture.unreachable-layer` публикуется один раз на каждый объявленный слой — или на каждый конкретный инстанс, развёрнутый из шаблона, — чьи паттерны не совпали **ни с одним классом, ни с одним концом ребра зависимости** за прогон **и** который не мог бы владеть ни одним символом, чьё назначение прогон оставил под сомнением. Слой, на который прогон не смог ответить, не объявляется не совпавшим ни с чем — к такому выводу прогон не пришёл, — как и слой, который владел бы символом, если бы его убрал неразрешённый `exclude:` более раннего слоя; оба называет [`architecture.doubted-assignment`](#doubted-assignment). Это диагностика конфигурации (см. врезку в разделе [Режимы покрытия](#режимы-покрытия-coverage)): она валит прогон безусловно, как только срабатывает, и не настраивается, не бейзлайнится и не подавляется `@qmx-ignore`. Три возможные причины:
...
Затенение строится только между совпадениями, которые прогон установил. Слой, чей `exclude:` не удалось разрешить для класса, ещё может этот класс потерять, поэтому для него он не затеняет более поздний слой и сам не затенён; сомнение публикует [`architecture.doubted-assignment`](#doubted-assignment).
