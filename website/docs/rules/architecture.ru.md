...
учитываются даже без единого ребра зависимости. Это независимо от самого
`architecture.layer-violation`: диагностика покрытия публикуется под отдельным
именем правила `architecture.coverage`, поэтому её можно бейзлайнить, подавлять
и фильтровать отдельно.
...
| `warn`             | Одна сводная диагностика `architecture.coverage` за прогон с severity `Warning`, со списком примеров неклассифицированных классов. При дефолтном `fail_on: error` прогон не валится, но валится, если выставить `fail_on: warning` или строже. |
| `error`            | То же самое, но с severity `Error` — подходит для fail-closed контроля владения всем анализируемым кодом в CI.                                                                                                                                 |

!!! note
    До этого релиза режим `warn` выдавал severity `Info` — расхождение с названием режима, из-за которого диагностика была невидима для `fail_on: warning`. Теперь `warn` выдаёт `Warning`, как и подразумевает название. Если вы полагались на то, что `coverage: warn` молчит при `fail_on: warning`, переключитесь на `coverage: ignore` либо поднимите `fail_on` до `error`.
...
`architecture.unreachable-layer` (severity по умолчанию `Info`, настраивается опцией правила `unreachable_layer_severity` — см. [Настройки](#layer-violation-options)) публикуется один раз на каждый объявленный слой — или на каждый конкретный инстанс, развёрнутый из шаблона, — чьи паттерны не совпали **ни с одним классом, ни с одним концом ребра зависимости** за прогон. Три возможные причины:
...
`architecture.empty-template` (severity по умолчанию `Warning`, настраивается опцией правила `empty_template_severity` — см. [Настройки](#layer-violation-options)) публикуется один раз на каждый шаблонный слой, развернувшийся в **ноль** конкретных инстансов — обычно из-за опечатки в шаблоне паттерна, исключённого модуля или односегментного `{var}`, использованного там, где binding охватывает несколько сегментов namespace (используйте `{var:**}` для cross-segment-захватов).
...
`architecture.potential-shadow` (severity по умолчанию `Info`, настраивается опцией правила `potential_shadow_severity` — см. [Настройки](#layer-violation-options)) ловит тихий режим отказа declaration-order-сопоставления: когда класс матчится несколькими слоями, побеждает только первый, и более ранние слои могут тихо отбирать классы, которые пользователь рассчитывал отдать более позднему, более узкому слою.
...
| `unreachable_layer_severity` | `info`       | Severity для диагностики `architecture.unreachable-layer` (см. [диагностику недостижимого слоя](#диагностика-недостижимого-слоя)). Допустимо: `info`, `warning`, `error`.            |
| `potential_shadow_severity`  | `info`       | Severity для диагностики `architecture.potential-shadow` (см. [диагностику потенциального затенения](#диагностика-потенциального-затенения)). Допустимо: `info`, `warning`, `error`. |
| `empty_template_severity`    | `warning`    | Severity для диагностики `architecture.empty-template` (см. [диагностику пустого шаблона](#диагностика-пустого-шаблона)). Допустимо: `info`, `warning`, `error`.                     |
...
rules:
  architecture.layer-violation:
    enabled: true
    severity: error
    # Поднять диагностику опечатки в `patterns:` до CI-гейтящей ошибки вместо
    # дефолтной info-диагностики. Каждая из трёх severity под-диагностик
    # независима от `severity` и друг от друга.
    unreachable_layer_severity: error
```

Три severity под-диагностик по умолчанию равны своим историческим значениям, так что добавление блока опций этого правила само по себе ничего не меняет, пока вы не переопределите одну из них. Поднятие `unreachable_layer_severity` (или `potential_shadow_severity`) до `error` — штатный способ завалить CI при перепутанном порядке или опечатке в объявлении слоёв, не поднимая заодно все остальные info-диагностики через `fail_on: info`.
...
- **Защитные сети, а не ошибки неоднозначности.** Прежний алгоритм на основе specificity отбраковывал неоднозначные конфигурации при загрузке. При declaration-order-сопоставлении неоднозначности не существует — порядок её разрешает — но пользователь всё ещё может **перепутать порядок**. Две диагностики ловят это — обе по умолчанию severity `Info`, но настраиваются независимо (`unreachable_layer_severity` / `potential_shadow_severity`, см. [Настройки](#layer-violation-options)): `architecture.unreachable-layer` (слой ничего не захватил) и `architecture.potential-shadow` (более ранний слой тихо отобрал классы у более позднего). См. отдельные разделы выше.
