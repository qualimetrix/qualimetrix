...
6. **One way to run a child process** — `Qualimetrix\Subprocess\ChildProcess` at `scripts/subprocess/ChildProcess.php` drains every stream concurrently; any other spawn must be declared and justified in `governance/SubprocessDrain/` (see [ADR 0067](adr/0067-subprocess-read-discipline.md))
