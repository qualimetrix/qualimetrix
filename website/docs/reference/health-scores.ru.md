...
!!! warning "Метрики, которых нет на уровне"
    ...
    Тернарный оператор читает только ту ветвь, которую выбрал. `m["size.method-count"] > 0 ? 7 : m["cohesion.tcc"]` читает `cohesion.tcc` лишь у класса без методов, поэтому ключ, который читает только одна ветвь, никогда не даёт ошибку конфигурации: каждый символ судится по ветви, выбранной его собственными значениями, а символ, чья ветвь читает отсутствующий у него ключ, пропускается с тем же предупреждением. Правая часть `and` / `or` судится так же. Условие выполняется всегда, поэтому чтение без `??` в нём считается как в арифметике — отсутствующая метрика выбрала бы ветвь ни на чём (`null > 0` ложно); защищайте его через `??`. Ключ, который читают обе ветви, считается прочитанным.
