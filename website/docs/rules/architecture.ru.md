...
!!! warning "Диагностика конфигурации, а не долг кода"
    `architecture.coverage` — одна из четырёх архитектурных диагностик, которые
    сигнализируют об ошибке в самой *конфигурации* архитектуры, а не о долге в
    анализируемом коде: остальные три — `architecture.unreachable-layer`,
    `architecture.potential-shadow` и `architecture.empty-template`. Все
    четыре валят прогон безусловно, как только срабатывают: `fail_on` для них
    не учитывается вообще, даже `fail_on: none`, и ни одну из четырёх нельзя
...
Шаблон, развернувшийся в ноль инстансов, **тихо отключает** связанную с ним политику, поэтому — как и три другие диагностики конфигурации — он валит прогон безусловно, а не ждёт severity или настройки `fail_on`; см. врезку в разделе [Режимы покрытия](#режимы-покрытия-coverage). Три типичные причины:
...
Четыре архитектурные диагностики конфигурации — `architecture.coverage`,
`architecture.unreachable-layer`, `architecture.potential-shadow` и
...
Чтобы подавить нарушение слоя, адресуйте точный канал: `@qmx-ignore architecture.layer-violation`. Более короткой формы нет — сопоставление по префиксу убрано, поэтому голое `@qmx-ignore architecture` — это ошибка, а не замена всей семьи. `architecture.*` тоже соблазнительна, но неверна: она заодно затронула бы не связанное с этим правило `architecture.circular-dependency`, а поскольку у `architecture.layer-violation` только один канал (он сам), `architecture.layer-violation.*` не совпадает ни с чем и падает ошибкой. «Все каналы политики слоёв» поэтому невыразимы по замыслу. Правило публикует пять каналов, но остальные четыре несут собственные имена правил (`architecture.coverage`, `architecture.unreachable-layer`, `architecture.potential-shadow`, `architecture.empty-template`), так что ни один селектор их не охватывает, — а поскольку все четыре суть ошибки конфигурации, которые никакое подавление не принимает, охватывающему их селектору всё равно нечего было бы делать.
