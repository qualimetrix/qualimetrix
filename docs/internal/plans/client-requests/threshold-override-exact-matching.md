...
> отмечены отдельно.
> Д2 и Д3 закрыты подложкой. Решение №3 этого плана отменено ею же (см. §ниже). Дополнительно: форма
> `ruleName#violationCode` у `@qmx-ignore` теперь **поддержана** (раньше молча обрезалась по `#`),
> а `@qmx-threshold` на выключенном правиле стал валидным и тихим.
