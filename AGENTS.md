...
| `scripts/generate-modular-architecture-test-inventory.php` — the scan-scope literal in `inventoryScanScope()` (the `git ls-files` path list feeding discovery). Loud since `assertEveryTestDirectoryIsScanned()` began reading it: a test-shaped directory git carries and the scope misses is refused by name. Still silent for a root whose test directory is not test-shaped — its own row, two below | loudly       |
| `scripts/generate-modular-architecture-test-inventory.php` — `NON_SCANNED_TEST_DIRECTORIES`. Not an address a root visits: it is where a test-shaped directory that is **not** a root is excused. `assertEveryTestDirectoryIsScanned()` refuses an entry that is stale, redundant or reasonless, and refuses a test-shaped directory the scan does not reach                                             | loudly       |
| a root whose test directory is **not** test-shaped (the `governance/` shape) — outside that sweep's population entirely. Declared under a `<testsuite>` the suite classifier catches it; undeclared, nothing does (ADR 0067 records this as an open residue)                                                                                                                                             | **silently** |
...
| a development root declared anywhere other than `composer.json` `autoload-dev` `psr-4` — `src/` importing it is not refused, because the ban is derived from that section alone                                                                                                                                                                                                                          | **silently** |
...
| the mover's `createIsolatedProject()` helper (e.g. `scripts/modular-architecture/tests/ModularArchitectureGeneratorRefusalTest.php`) — its copy list must contain every root the tracked config declares, or PHPUnit exits 2 inside the scratch project, **and** every path the generator's own literals name, or a planted refusal is replaced by a generic one                                         | loudly       |
...
| `.dockerignore` — a directory-shaped `TOOLING_TEST_ROOT_OWNERS` key needs its own exact line, as the last one matching it; `DockerBuildContextExcludesToolingTestRootsTest` names a missing or negated one                                                                                                                                                                                               | loudly       |
| `.dockerignore` for anything else, and `scripts/init-environment.sh`                                                                                                                                                                                                                                                                                                                                     | **silently** |
