...
`architecture.unreachable-layer` публикуется один раз на каждый объявленный слой — или на каждый конкретный инстанс, развёрнутый из шаблона, — чьи паттерны не совпали **ни с одним классом, ни с одним концом ребра зависимости** за прогон **и** который не мог бы владеть ни одним анализируемым классом, чьё назначение прогон оставил под сомнением. Слой, на который прогон не смог ответить об анализируемом классе, не объявляется не совпавшим ни с чем — к такому выводу прогон не пришёл, — как и слой, который владел бы анализируемым классом, если бы его убрал неразрешённый `exclude:` более раннего слоя; оба называет [`architecture.doubted-assignment`](#doubted-assignment). Символ вне анализируемых путей ни один слой от этой диагностики не спасает: прогон такой символ не читает, поэтому критерий `implements` / `extends` / `attributes` остаётся о нём без ответа в любом прогоне — в том числе критерий с опечаткой, — и одного ребра в vendor-код хватило бы, чтобы такая опечатка не нашлась никогда. Вместо этого находка говорит, сколькими символами вне анализируемых путей слой мог бы владеть. Это диагностика конфигурации (см. врезку в разделе [Режимы покрытия](#режимы-покрытия-coverage)): она валит прогон безусловно, как только срабатывает, и не настраивается, не бейзлайнится и не подавляется `@qmx-ignore`. Три возможные причины:
