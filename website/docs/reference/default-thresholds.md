...
| `annotation.unused-directive`      | Info (configurable via `unused_directive_severity`) | enabled | Ordinary finding, not a configuration error: the directive is valid but nothing it addressed fired this run. Can be baselined or excluded like any other channel; it is the one channel no `@qmx-ignore` may address. |
