...
- **Надбавка за вложенность:** `if`, тернарный оператор, `switch`, циклы и `catch` добавляют ещё текущий уровень вложенности. `elseif`, `else`, `goto` и переходы с номером — нет.
...
- **Именованные функции и методы анонимных классов**, объявленные внутри метода, измеряются как отдельные единицы с уровня вложенности 0.
...
!!! info "Отклонение от оригинальной спецификации"
    ...
    - **Рекурсия:** обнаруживается только прямой вызов самого себя (`$this->method()`,
      `self::`/`static::method()` или функция, вызывающая себя). Спецификация засчитывает
      и каждый метод косвенного цикла (A вызывает B, B вызывает A); Qualimetrix — нет.
