...
Если пути не указаны, Qualimetrix автоматически определит их по PSR-4 корням секции `autoload` вашего `composer.json`. `autoload-dev` не включается: тестовый код анализируется, только когда его путь назван явно (`bin/qmx check src/ tests/` или `paths:` в `qmx.yaml`), или когда вы считаете его частью проекта через [`--include-autoload-dev`](#--include-autoload-dev).
...
- прогон без путей анализирует PSR-4 корни `autoload-dev` рядом с корнями `autoload`;
...
один набор опций по нескольким форматам, продолжает работать.
...
Файл, который нельзя записать (его каталога нет или он недоступен для записи), отклоняется с
кодом 3 до начала анализа, как и пустое `--profile=`. Запись, не удавшаяся уже после прогона,
отклоняется так же и никогда не сообщается рядом с завершённым прогоном.
