...
| Недостижимый слой       | `architecture.unreachable-layer`   | Info                                     | включено (срабатывает только с `architecture.layers`) | Один Info-диагностик на каждый объявленный слой, паттерны которого не совпали ни с одним классом. Ловит ситуацию, когда более широкий паттерн раньше в списке тихо перехватывает класс у более позднего слоя. Run с одними Info по умолчанию завершается с кодом 0; используйте `fail_on: info`, чтобы это блокировать сборку.                                                        |
| Потенциальное затенение | `architecture.potential-shadow`    | Info                                     | включено (срабатывает только с `architecture.layers`) | Поиск перекрытия слоёв по фактическим классам: для каждой пары (assigned, shadowed) — один диагностик. Run с одними Info по умолчанию завершается с кодом 0; используйте `fail_on: info`, чтобы это блокировать сборку.                                                                                                                                                               |
| Пустой шаблон           | `architecture.empty-template`      | Warning                                  | включено (срабатывает только с шаблонными слоями)     | Один Warning на каждый шаблонный слой, развернувшийся в ноль конкретных инстансов — фактически отключает привязанную к нему политику. Типичные причины: опечатка в шаблоне паттерна, все кандидаты исключены, или односегментный `{var}` там, где нужен `{var:**}`. Default `fail_on: error` не блокирует сборку; поднимите до `fail_on: warning`, чтобы CI падал на пустых шаблонах. |
| Покрытие слоёв          | `architecture.coverage`            | Warning или Error (по режиму `coverage`) | выключено (`coverage: ignore`)                        | Один агрегированный диагностик, когда `architecture.coverage` установлен в `warn` или `error` и есть зависимости с неклассифицированными концами. Серьёзность соответствует выбранному режиму.                                                                                                                                                                                        |
