...
**`attributes`, `implements` и `extends` отвечают по анализируемому множеству.** Эти три критерия читают факты объявлений, собранные прогоном, поэтому «транзитивно» достаёт ровно настолько, насколько достаёт `paths`. Собственные родители и интерфейсы класса известны всегда, даже когда это вендорные типы — ребро зафиксировано от самого анализируемого класса, — поэтому критерий, называющий прямого предка, работает как обычно. Критерию, называющему что-то выше по цепочке, нужно, чтобы внутри `paths` оказалось и каждое промежуточное звено; если звена нет, прогон не отвечает на критерий вместо того, чтобы ответить «нет». То же верно и для класса, который прогон не анализировал вовсе, — вендорного типа, до которого дотянулись только как до дальнего конца ребра зависимости: о его супертипах тоже ничего не собрано. Неразрешённый слой не отзывает совпадение у более позднего — назначение остаётся, а сомнение публикуется рядом, — но когда не совпал ни один слой, такой класс докладывается как *неразрешимый*, а не как неклассифицированный: `architecture.coverage-gap` считает его отдельно (см. [Режимы покрытия](#режимы-покрытия-coverage)), а `debug:layer-assignment` печатает `(undecided)` (см. [Инспекция назначения слоя для одного класса](#debug-layer-assignment)). `patterns` и `suffix` читают только собственное имя класса и разрешимы всегда.
...
`(undecided)` и `(no layer)` — два разных факта, и общей формы у них нет: первое означает, что прогон не смог ответить, второе — что каждый объявленный критерий ответил «нет». Когда более поздний слой всё же совпадает, назначение показывается как обычно, а слой, оставшийся без ответа, называется рядом строкой `Could not be decided:`.

`--format=json` несёт те же три состояния. Ключ `undecided` присутствует всегда и перечисляет слои, на которые этот прогон не смог ответить; `assigned: null` вместе с непустым `undecided` означает «не смог сказать», а не «класс не заявлен ни одним слоем», — поэтому потребителю, ветвящемуся только по `assigned`, нужно читать и `undecided`.
...
- **Концы вне слоёв молча игнорируются** для целей layer-violation. Их количество отдельно публикуется через режим `coverage-gap`, который делит его на два: классы, о которых каждый объявленный критерий ответил «нет», и классы, для которых какой-то критерий вообще не удалось разрешить.
