...
| `identicalSubExpression.{type}.count`                                                  | Code Smell      | Method    | Number of identical sub-expression findings per type (`identical_operands`, `duplicate_condition`, `identical_ternary`, `duplicate_match_arm`) |
| `identicalSubExpression.{type}.line.{i}`                                               | Code Smell      | Method    | Line number of each identical sub-expression finding                                                                                           |
| **Design**                                                                             |                 |           |                                                                                                                                                |
...
| `security.sqlInjection.count`, `security.xss.count`, `security.commandInjection.count` | Security        | File      | Security pattern counts (SQL injection, XSS, command injection)                                                                                |
| `security.sensitiveParameter.count`                                                    | Security        | File      | Sensitive parameters without `#[\SensitiveParameter]` attribute                                                                                |
