...
- `RuleInterface::getCliAliases()` removed. CLI aliases are now declared via the repeatable class-level attribute `#[CliAlias('alias', 'optionName')]` and read by `CliAliasReader::read(class-string)` via reflection. Existing rules in this codebase migrated; custom rules in extension code must drop the `getCliAliases()` method and replace it with attributes on the class.
...
- `qmx.yaml.example` now includes a commented-out `architecture:` stanza demonstrating layers (with multi-criterion membership, `exclude:`, a template layer, and a vendor layer), `allow:` (plain, captured same-instance, and long-form with `relations:` filter), `coverage`, and `max_expanded_layers` as a teaching reference for the Phase 2 surface.
- `architecture.layers` validation errors now carry richer context: duplicate-pattern collisions include the config-load index of both colliding layers, `architecture.max_expanded_layers` rejections show the default ceiling and remediation hint, and criterion-list shape errors distinguish "associative map where a list was expected" from generic non-list inputs.
- Architecture template expansion (`ArchitectureProcessor::prepare()`) is now skipped when `architecture.layer-violation` is disabled — the only consumer of the prepared configuration. Symmetric with the existing duplication-detection skip when `duplication.code-duplication` is disabled. Disabling architecture rules now yields the expected runtime-cost reduction.
...
### Fixed
- `@qmx-threshold` annotations on `design.type-coverage`, `design.god-class`, and `design.data-class` previously had no effect — the Options classes did not implement `ThresholdAwareOptionsInterface` and the rules did not consult per-symbol overrides. The three Options now implement the interface and the rules apply overrides per class: `type-coverage` overrides all three dimensions (param/return/property) uniformly, `god-class` maps `warning` to `minCriteria`, `data-class` maps `warning` to `wocThreshold` and `error` to `wmcThreshold`.
...
- `architecture.allow` source and target selectors now reject `[` brackets at config-load time with an actionable hint suggesting `{var}` capture-variable syntax. Previously brackets were silently treated as glob character classes, leading to semantic divergence between user intent (capture) and actual behaviour. Layer names themselves are unaffected — `LayerDefinition` already rejected `[` via its `[a-z][a-z0-9_-]*` regex.
- `debug:layer-assignment` now honours `memory_limit` from `qmx.yaml`. Phase 4.5 made the debug command run a full Discovery + Collection pass (to match `qmx check` byte-for-byte), but the per-run `RuntimeConfigurator::configure()` hook — which applies the YAML `memory_limit` — was wired only into `CheckCommand`. On any non-trivial codebase the parallel-worker pool exhausted the default 128MB and crashed mid-collection. The configurator is now invoked from `LayerAssignmentCommand` as well.
