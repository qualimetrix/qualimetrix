...
### Changed
- Every rule now declares its own estimated remediation time (in minutes) on its own class, alongside its documentation page and default thresholds. See [Remediation Time](website/docs/reference/remediation-time.md) for the full table. `coupling.class-rank` debt is no longer scaled by overshoot: its rank is a project-wide normalised PageRank rescaled per class count, so a stored value is not comparable across runs — it now reports its flat base estimate like every other `occurrence`-shaped channel.
...
- New rule `annotation.directive` reports inline `@qmx-*` directives that address nothing, can never apply, or no longer do anything. Three of its channels (`annotation.unresolved-directive`, `annotation.unsupported-threshold`, `annotation.invalid-threshold`) are configuration errors that end the run regardless of `fail_on`; `annotation.unused-directive` reports a directive that is valid but no longer fires, at a configurable `unused_directive_severity` (default `info`). Diagnostics name what *can* be addressed, so `@qmx-threshold coupling.cbo.class` is told that the string is a channel of the rule `coupling.cbo`.
- `@qmx-ignore`, `@qmx-ignore-next-line`, and `@qmx-ignore-file` now accept the explicit `ruleName#violationCode` pair to name a channel unambiguously, e.g. `@qmx-ignore complexity.cyclomatic#complexity.cyclomatic.callable`.
- Per-rule `exclude_namespace_channels` keys accept the explicit `ruleName#violationCode` pair too, so the channel grammar is the same everywhere it is documented to be. The pair used to parse as a name containing a `#`, address nothing, and be refused with advice that contradicted the documentation.
...
- New `architecture.unassigned-class` gate counts analysed class-like declarations that landed outside every declared layer, without the vendor dependency-edge ends that drown `architecture.coverage`. Off by default; set `unassigned_class: ignore|warn|error` on `architecture.layer-violation` (CLI `--layer-violation-unassigned-class`). It reports an absolute count, so a baseline can ratchet it down.
...
### Fixed
- `cohesion.lcom` no longer counts `__construct`/`__destruct` as graph vertices. A constructor built entirely from promoted properties (`public function __construct(private array $x) {}`) never emits a property-access node, so it previously landed in the LCOM graph as an isolated vertex and inflated LCOM by one for the large majority of PHP 8+ constructors — the same treatment TCC/LCC already gave constructors and destructors.
...
- **`@qmx-ignore` requires the full channel name.** For a rule that emits more than one channel, the rule name alone no longer suppresses anything. Affected rules and their channels: `coupling.cbo` (`coupling.cbo.class`, `coupling.cbo.namespace`), `coupling.instability` (`.class`, `.namespace`), `complexity.cyclomatic` (`.callable`, `.class`), `complexity.cognitive` (`.callable`, `.class`), `complexity.npath` (`.callable`, `.class`), `design.type-coverage` (`.param`, `.property`, `.return`). Replace `@qmx-ignore complexity.cyclomatic` with the exact channel that fires on that symbol — `complexity.cyclomatic.callable` on a method, `complexity.cyclomatic.class` on a class — or with `complexity.cyclomatic.*` when both are meant. Prefer the exact channel: it says what it does.
