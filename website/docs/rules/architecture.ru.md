...
Блок `exclude:`, на который прогон не может ответить, — критерий `extends` / `implements` / `attributes` для класса, чья цепочка наследования выходит за `paths`, — класс не удаляет: он остаётся в слое, а сомнение публикуется так, как описано в разделе [Принадлежность за пределами namespace-паттернов](#принадлежность-за-пределами-namespace-паттернов). Удаление оставило бы его рёбра без проверки allow-списком — это худшая из двух ошибок.
...
Когда назначение стоит на слое, на который прогон не смог полностью ответить, — более раннем слое, оставшемся без ответа, или собственном `exclude:` назначенного слоя, — ещё одно предложение считает такие классы, и диагностика выдаётся ради них даже тогда, когда вне слоёв нет ничего:
...
Рекомендация при этом говорит, что каждая правка делает с неразрешимой частью:
