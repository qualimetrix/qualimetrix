...
Сбор доказательств, декодирование вхождений и правила находятся вместе в
возможности `Analysis\Evidence\CodeSmell`. Идентификаторы правил и конфигурация
не изменяются.

Имена функций сравниваются так, как они написаны в вызове. Импорты не
разрешаются, поэтому функция, вызванная через псевдоним (`use function var_dump
as vd; vd($x);`), не распознаётся правилами, которые ищут функцию по имени
...
Доказательство -- объявленный тип: `bool`, `?bool` или объединение, содержащее `bool`. Параметр без типа не помечается, даже с булевым значением по умолчанию (`$overwrite = false`) или PHPDoc `@param bool`.
...
Вызовы в режиме возврата (`var_export($value, true)`, `print_r($value, return: true)`) и вызовы внутри методов отладочного API (`dump()`, `dd()`, `debug()`, `dumpRawSql()`, `dumpSql()`, `debugInfo()`, `__debugInfo()`) не помечаются. Позиционный `true` означает режим возврата только у `print_r()` и `var_export()` — двух функций, у которых есть параметр `$return`: `var_dump($value, true)`, `dd($value, true)` и `dump($value, true)` выводят оба аргумента и помечаются.

`debug_backtrace()` не помечается: он возвращает трассировку, а не печатает её, и часто используется в обработке ошибок.
...
`catch`, в котором есть только комментарий, всё равно пуст: комментарий объясняет молчание, но не прекращает его.

**Цепочка попыток.** Одна форма не помечается: `foreach`, который перебирает элементы, пока один не сработает. `try` должен быть непосредственной инструкцией тела цикла и должен уметь завершить поиск при успехе: в нём есть `return` или `continue`, пропускающий инструкции после `try`, -- на верхнем уровне `try` или внутри его веток `if`:

```php
foreach ($this->resolvers as $resolver) {
    try {
        return $resolver->resolve($key);  // успех завершает поиск
    } catch (ResolverException $e) {
        // неудача означает «попробовать следующий»
    }
}
```

`try`, вложенный глубже в тело цикла, `try` внутри замыкания и `try`, из которого есть только `continue` без инструкций после `try` (и поэтому он ничего не пропускает), помечаются как обычно.
...
4. **Подавите находку с причиной,** если исключение игнорируется намеренно: комментарий в `catch` находку не снимает, а `@qmx-ignore code-smell.empty-catch -- <причина>` снимает.
...
Обнаруживает использование `exit()` и `die()`, включая полностью квалифицированный вызов функции `\exit()` / `\die()`, появившийся в PHP 8.4. Эти функции немедленно завершают весь PHP-процесс, что:
...
Обнаруживает прямой доступ к суперглобальным переменным PHP: `$_GET`, `$_POST`, `$_REQUEST`, `$_SERVER`, `$_SESSION`, `$_COOKIE`, `$_FILES`, `$_ENV` и `$GLOBALS`. Читается только обычное имя переменной: запись через переменную переменную (`${'_GET'}`, `$$name`) не обнаруживается.
...
Обнаруживает идентичные подвыражения, указывающие на ошибки копирования или логические баги. Правило выявляет пять паттернов:

1. **Одинаковые операнды в бинарных операциях** (`identical_operands`) -- одно и то же выражение по обе стороны оператора (например, `$a === $a`, `$a - $a`, `$a && $a`).
2. **Дублирующиеся условия в цепочках if/elseif** (`duplicate_condition`) -- одно и то же условие проверяется дважды, что делает вторую ветку мёртвым кодом. `else`, в котором нет ничего, кроме другого `if` (`else if` или `else { if (...) }`), продолжает цепочку.
3. **Одинаковые ветки тернарного оператора** (`identical_ternary`) -- тернарный оператор, в котором ветки «истина» и «ложь» совпадают, что делает условие бессмысленным.
4. **Дублирующиеся условия в match** (`duplicate_match_arm`) -- повторяющиеся условия в выражении `match`, где выполнится только первый вариант.
5. **Дублирующиеся case в switch** (`duplicate_switch_case`) -- одно и то же значение `case` повторяется в `switch`, где выполнится только первый вариант.

Операторы с легитимным использованием одинаковых операндов не помечаются: `+`, `*`, `**`, `.`, `&`, `|`, `<<`, `>>`.

Побочные эффекты учитываются по-разному для разных паттернов:

- **Операнды и ветки тернарного оператора** с побочными эффектами (вызовы функций и методов, присваивания, инкременты и т.д.) исключаются, так как два вычисления могут дать разные результаты.
- **Условия** (паттерны 2, 4 и 5) сравниваются вместе с вызовами: повторный вызов в цепочке условий почти всегда ошибка копирования, как в примере с `$order->isPaid()` ниже. Цепочка, которая намеренно повторяет вызов с побочным эффектом (`$stack->pop()`), помечается, и её нужно подавить inline-директивой с причиной.
...
5. **Дублирующиеся case в switch:** Удалите недостижимый `case` или исправьте его значение.
...
- **Магические методы:** классы с `__call`/`__callStatic` пропускают проверку методов; классы с `__get`/`__set` пропускают проверку свойств. Магический метод учитывается при любой видимости и в любом регистре, а также когда его объявляет трейт из того же файла
...
- **Анализируемые типы:** классы и перечисления; интерфейсы и трейты не анализируются
- **Паттерны доступа:** распознаёт `$this->method()`, `self::method()`, `static::method()`, доступ к свойствам и константам. Имена методов сравниваются без учёта регистра, как их разрешает PHP; имена свойств и констант регистрозависимы
- **Callable:** литеральный callable-массив считается использованием метода: `[$this, 'method']`, `[self::class, 'method']`, `[static::class, 'method']`, `[__CLASS__, 'method']`, в том числе внутри `Closure::fromCallable()` или `array_map()`. Метод, названный переменной (`[$this, $name]`, `$this->$name()`) или строкой вида `'self::method'`, не распознаётся, и такой метод считается неиспользуемым
- **Рекурсия:** приватный метод, который вызывается только из собственного тела, считается неиспользуемым; callable-массив с именем метода внутри его собственного тела — тоже ссылка на себя
...
Каждый список инструкций метода, функции, замыкания или хука свойства проверяется отдельно: само тело и каждый вложенный блок в `if`/`elseif`/`else`, циклах, `try`/`catch`/`finally`, ветках `switch` и простых блоках. Мёртвая инструкция считается один раз, без заглядывания внутрь, а метка `goto` снова делает достижимыми инструкции после неё. `break` после `return` в ветке `switch` тоже недостижим и учитывается.

Терминальность определяется для каждого списка инструкций отдельно, без анализа потока между ветками: код после `if`/`else`, у которого каждая ветка завершается `return`, не помечается.
