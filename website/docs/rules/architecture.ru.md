...
Классы, которые убрал clause, достаются следующему совпавшему с ними слою, и этот слой может объявить **тот же паттерн**. Это простейшая форма выреза:
...
- name: repo-doctrine
  patterns: ['App\Repository\**']   # получает ровно то, что исключил repo-plain
```

В остальных случаях два слоя с одинаковым паттерном отклоняются при загрузке: первый забирает каждый класс, который называет паттерн, и второму не достаётся ни одного. Место освобождает только `exclude:` **более раннего** слоя — собственный `exclude:` более позднего сужает то, что берёт он сам, а не то, что уже взял ранний, — и ещё ранний слой с `match: all`, сужающий паттерн другим критерием. Третий слой с тем же паттерном отклоняется за слоем, получившим вырезанные классы. Досталось ли получателю хоть что-то — факт о коде, и о нём во время прогона сообщает [`architecture.unreachable-layer`](#диагностика-недостижимого-слоя). Слои, которые загрузка принимает на повторённом паттерне, — ровно те, о которых не сообщает [`architecture.potential-shadow`](#диагностика-потенциального-затенения): проигрывать классы, оставшиеся у раннего слоя, — это то, ради чего получатель объявлен.
...
- **Он судится только на прогоне, который способен судить.** Как и прочие
  ...
  покрывающих всё, что `composer.json` объявляет в `autoload`, — корни
  `psr-4` и `psr-0`, записи `classmap` и `files` наравне, — а с
  [`--include-autoload-dev`](../usage/cli-options.ru.md#--include-autoload-dev)
  и в `autoload-dev`. На более узком прогоне канал молчит, а
  [охват проекта](../usage/output-formats.ru.md#project-scope-in-every-format)
  в отчёте называет его среди каналов, которые не судились. Проект, чей манифест ничего не объявляет в
  секциях, которые учитывает прогон, не имеет иного проекта, кроме названных
  путей, поэтому канал судит эти пути как весь проект.
...
Неразрешённый слой никогда не отзывает совпадение, поэтому символ может стоять в слое, пока слой, от которого зависит назначение, остался без ответа: слой, объявленный *до* назначенного, чей критерий `extends` / `implements` / `attributes` не удалось разрешить, или собственный `exclude:` назначенного слоя. Назначение остаётся, его рёбра судятся; прогон не может сказать лишь одного — изменил бы его слой, оставшийся без ответа, или нет. Слой, объявленный *после* назначенного, сомнением не считается — побеждает первое совпадение, так что владеть символом он не мог, — кроме случая, когда без ответа остался собственный `exclude:` назначенного слоя: блок может убрать символ, и тогда им владеет более поздний слой.

`architecture.doubted-assignment` считает такие символы, а рядом с ними — символы, которые не попали ни в один слой только потому, что какой-то слой не смог о них ответить: их рёбра не судит ни один allow-список, а принадлежать они могут этому слою. Это сведение, а не пропуск: находка выдаётся с severity **info**, поэтому прогон она не валит никогда, что бы ни говорил `fail_on`, и публикуется при любом режиме `coverage-gap` — она говорит о том, верно ли членство, а не о том, сколько кода покрывают слои. Одна находка за прогон разбивает каждый счёт на анализируемые классы и символы вне анализируемых путей, называет каждый слой, не сумевший ответить, с числом оставленных им под сомнением символов, и приводит примеры каждого вида — не больше 10 на вид, по имени:
...
Когда неразрешённый `exclude:` удерживает символ, которым владел бы более поздний слой, если бы блок этот символ убрал, такой слой тоже называется — отдельным списком: `Layers that would own some of them if an unanswered "exclude" removed them: "repos" (1 assigned in doubt).` Вместе два списка называют каждый слой, который [`architecture.unreachable-layer`](#диагностика-недостижимого-слоя) пропускает из-за сомнения.
...
- Для символа вне анализируемых путей всё зависит от того, чей это код. «Вне анализируемых путей» — факт о прогоне: прогон по одному каталогу оставляет снаружи и собственные классы проекта, а их сомнение снимает анализ — расширение `paths` или прогон по всему проекту. Класс зависимости — обычно вендорный класс на дальнем конце ребра — решается `patterns`-слоем для его неймспейса, объявленным *до* слоя, не сумевшего ответить. `debug:layer-assignment` не поможет ни в том, ни в другом случае: класс, который прогон не анализировал, команда отвергает, — поэтому слои называет сама находка.

Прогон по части проекта поэтому даёт другие сомнения, чем прогон по всему проекту, а не меньше: собственный класс, который полный прогон решает, может оказаться под сомнением, если прогон его не читал.
...
Канал публикует `architecture.layer-violation`, поэтому отключение этого правила глушит и его. Чтобы отказаться только от этого канала, перечислите `architecture.doubted-assignment` в `disabled_rules` (или передайте `--disable-rule=architecture.doubted-assignment`).
...
`architecture.unreachable-layer` публикуется один раз на каждый объявленный слой — или на каждый конкретный инстанс, развёрнутый из шаблона, — чьи паттерны не совпали **ни с одним классом, ни с одним концом ребра зависимости** за прогон **и** который не мог бы владеть ни одним анализируемым классом, чьё назначение прогон оставил под сомнением. Слой, который владел бы анализируемым классом, если бы его убрал неразрешённый `exclude:` более раннего слоя, не объявляется не совпавшим ни с чем — к такому выводу прогон не пришёл. Не объявляется и слой, на который прогон не смог ответить об анализируемом классе, пока хоть один тип, названный его критериями `attributes` / `implements` / `extends`, прогону известен: объявлен в анализируемых путях, объявлен самим PHP, встретился на любом конце ребра зависимости или объявлен в composer-инсталляции анализируемого проекта — его ищут по `vendor/composer/installed.json`, собственному `autoload` проекта и файлам, на которые они указывают, читая их как данные и никогда не загружая. Оба слоя называет [`architecture.doubted-assignment`](#doubted-assignment). Неизвестного прогону типа для этого мало: класс, чей родитель лежит вне анализируемых путей, оставляет без ответа любой такой критерий, в том числе критерий с опечаткой, и на любом проекте, где анализируемый класс наследует vendor-код, опечатка иначе не нашлась бы никогда. Символ вне анализируемых путей тоже ни один слой от этой диагностики не спасает: прогон такой символ не читает. Находка говорит, что осталось за скобками, теми словами, которые для этого верны: о скольких символах слой не смог ответить и — если он не называет ни одного известного прогону типа — какие это типы и спрашивали ли инсталляцию: опечатка или неустановленный пакет; либо со сколькими символами вне анализируемых путей совпали его критерии, пока их держит более ранний слой через `exclude:`, на который прогон не может ответить. Поэтому критерий, называющий тип, до которого доходит только vendor-цепочка, — `implements: ['Doctrine\Persistence\ObjectRepository']` над репозиториями, наследующими `ServiceEntityRepository`, когда анализируемый код нигде не называет этот интерфейс, — сохраняет свой слой, пока пакет установлен: инсталляция объявляет интерфейс, и слой вместо ошибки называет [`architecture.doubted-assignment`](#doubted-assignment). Инсталляция говорит только, что тип существует; реализует ли его репозиторий, по-прежнему без ответа — vendor-цепочку прогон не проходит. Если пакет не установлен, слой сообщается. Диагностику судят только на прогоне, который может её судить: как и [`architecture.unmatched-exclude`](#unmatched-exclude), ей нужны пути, покрывающие всё, что `composer.json` объявляет в `autoload`. На прогоне по части проекта — `qmx check src/Web` — слой, все классы которого вне среза, там ни с чем не совпадает, хотя владеет кодом в другом месте, поэтому диагностика молчит; о том, что прогон частичный, в stderr говорит предупреждение, что анализируемые пути покрывают не все autoload-записи, а в любом формате — [охват проекта](../usage/output-formats.ru.md#project-scope-in-every-format) в отчёте: `narrowed`, и этот канал среди тех, что не судились. Проект, чей `composer.json` не объявляет читаемого production-autoload, или проект без него судится: без манифеста проект — это названные пути, поэтому запускайте его по всему коду — `qmx check src/Web` на таком проекте сообщит о каждом слое, чьи классы лежат в другом месте, а охват проекта в отчёте будет `unknown`. Это диагностика конфигурации (см. врезку в разделе [Режимы покрытия](#режимы-покрытия-coverage)): она валит прогон безусловно, как только срабатывает, и не настраивается, не бейзлайнится и не подавляется `@qmx-ignore`. Три возможные причины:
...
Как и [`architecture.unreachable-layer`](#диагностика-недостижимого-слоя), её судят только на прогоне, чьи пути покрывают всё, что `composer.json` объявляет в `autoload`: срез проекта не может показать, что модуля нет.
...
| Шире или равен (`App\**` выиграл у `App\Http\**`)           | Диагностика (кроме одной равной пары) |
| Несравнимы (см. ниже)                                       | Диагностика (консервативный дефолт)   |

Одна равная пара молчит: тот же паттерн, повторённый за слоем, который забирает не каждый класс, названный паттерном, — со `exclude:` или с `match: all`, сужающим паттерн другим критерием. Поздний слой получает то, что оставил ранний, — так записывается [вырез](#исключение-поддеревьев-внутри-слоя-exclude), — и загрузка слоёв принимает ровно такие повторы.
...
Затенение строится только между совпадениями, которые прогон установил. Слой, чей `exclude:` не удалось разрешить для класса, ещё может этот класс потерять, поэтому для него он не затеняет более поздний слой и сам не затенён; сомнение публикует [`architecture.doubted-assignment`](#doubted-assignment). Первое установленное совпадение при этом по-прежнему затеняет все более поздние, даже если перед ним класс удерживает такой слой: более поздние теряют класс, как бы ни ответил блок. Если `app` вырезает `App\Repository\**` неразрешимым `exclude:`, а за ним объявлены `repos` и более узкий `legacy`, диагностика сообщает `repos` → `legacy`, но не `app` → `repos`.
...
`(undecided)` и `(no layer)` — два разных факта, и общей формы у них нет: первое означает, что прогон не смог ответить, второе — что каждый объявленный критерий ответил «нет». Когда более поздний слой всё же совпадает или слой совпал, но на его `exclude:` не удалось ответить, назначение показывается как обычно, а слой, оставшийся без ответа, называется рядом строкой `Could not be decided:`, за которой идёт `The chain stops at:`. Во втором случае в строке стоит сам назначенный слой. Слой без ответа, объявленный *после* назначенного, не называется: побеждает первое совпадение, так что изменить назначение он не может, и [`architecture.doubted-assignment`](#doubted-assignment) такой класс тоже не считает. Исключение — назначенный слой, чей собственный `exclude:` остался без ответа: блок может убрать класс, поэтому называется и более поздний слой без ответа — вплоть до первого совпадения, которое прогон установил. Строка `Could be owned by:` рядом называет все слои, которые могли бы владеть классом, когда на них будет получен ответ.

Диагностическая подсказка следует правилу, по которому [`architecture.potential-shadow`](#диагностика-потенциального-затенения) строит свои пары, поэтому никогда не отсылает к затенению, о котором `check` не сообщает. Она строится только между совпадениями, которые прогон установил, и называет слой, которому класс проигрывает, — первое из них; это не назначенный слой, когда перед ним стоит неразрешённый `exclude:`. Более поздний слой шире этого — это идиома «узкий перед широким», а более позднее совпадение, чей собственный `exclude:` остался без ответа, может вовсе не совпасть; подсказка говорит об этом, а не отсылает к диагностике.

`--format=json` несёт те же три состояния. Ключ `undecided` присутствует всегда и перечисляет слои, на которые этот прогон не смог ответить и от которых зависит назначение, — все такие слои, когда не совпал ни один, и объявленные до первого совпадения, которое прогон установил, в остальных случаях; `assigned: null` вместе с непустым `undecided` означает «не смог сказать», а не «класс не заявлен ни одним слоем», — поэтому потребителю, ветвящемуся только по `assigned`, нужно читать и `undecided`. Ключ `contenders` перечисляет слои, которые могли бы владеть классом, когда на них будет получен ответ, и пуст всякий раз, когда пуст `undecided`. Ключ `chainStopsAt` тоже присутствует всегда: это непрочитанные прогоном объявления, на которых остановилась цепочка наследования класса; он пуст всякий раз, когда пуст `undecided`. Ключ `shadowed` перечисляет все совпадения после `shadowedBy` — первого совпадения, которое прогон установил: каждое проигрывает класс, как бы ни ответили слои без ответа, и несёт `reported: true`, когда о нём сообщает `architecture.potential-shadow`, — но никогда для совпадения, чей собственный `exclude:` остался без ответа: оно может вовсе не совпасть. `shadowedBy` — не назначенный слой, когда перед ним стоит неразрешённый `exclude:`, и равен `null`, когда `shadowed` пуст. Ключ `contendingMatches` в той же форме перечисляет все остальные совпадения после `assigned` — каждое совпадение, чей `exclude:` остался без ответа, и, если такое стоит перед ним, первое совпадение, которое прогон установил; кому из них достанется класс, зависит от неразрешённых клауз, поэтому у каждого `reported: false`. Вместе `contendingMatches` и `shadowed` — это все совпадения, которые текстовый отчёт перечисляет под `Would also match`.
