...
Это создаёт скрипт `.git/hooks/pre-commit`, который автоматически запускает Qualimetrix на staged-файлах перед каждым коммитом. Если обнаружены нарушения с уровнем `error`, коммит блокируется.
...
В Qualimetrix есть два отдельных механизма ограничения области:
...
Ограничивает набор файлов, которые Qualimetrix обрабатывает:
...
Это быстрее, так как парсится меньше файлов, но метрики уровня пространства имён и связанности могут быть неполными, так как Qualimetrix не видит полную картину.
...
git commit -m "refactor: simplify UserService"
# Qualimetrix запускается автоматически, блокирует коммит при ошибках
```
