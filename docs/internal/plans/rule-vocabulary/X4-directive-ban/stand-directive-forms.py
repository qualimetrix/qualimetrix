...
FORMATTER = REPO / "scripts" / "format-md-tables.py"


def write_markdown(path: Path, text: str) -> None:
    """Writes a generated page the way the repository stores one.

    The pre-commit hook runs `format-md-tables.py` over staged markdown, so a
    generator that writes unaligned tables dirties the tree on every run and
    `git status` stops being evidence. The stand therefore formats what it
    writes and then asks the formatter whether anything is left to do — a
    silent no-op here would reintroduce exactly the noise it removes.
    """
    path.write_text(text)
    formatted = subprocess.call([sys.executable, str(FORMATTER), "--in-place", str(path)])
    if formatted != 0:
        raise StandError(f"the markdown table formatter exited {formatted} on {path.name}")
    if subprocess.call([sys.executable, str(FORMATTER), "--check", str(path)]) != 0:
        raise StandError(f"{path.name} still needs formatting after the formatter ran over it")
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="x4-forms-") as temporary:
        ...
        if arguments.output:
            try:
                write_markdown(arguments.output, text + "\n")
            except StandError as error:
                print(f"stand-directive-forms: {error}", file=sys.stderr)
                return 3
