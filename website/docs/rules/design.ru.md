...
Родитель вне анализируемого пути разрешается его загрузкой --- автолоадером
самого процесса `qmx`. Поэтому та часть цепочки, которая выходит за пределы
вашего кода, учитывается только если её может загрузить установка, выполняющая
анализ, и глубина обрывается на первом предке, которого она загрузить не может.

На практике один и тот же класс может получить разную оценку в зависимости от
способа установки инструмента. Установленный как зависимость анализируемого
проекта, `qmx` делит с ним `vendor/` и видит всю цепочку. Запущенный из
автономной установки --- Docker-образ, phar, `composer global require` --- он не
видит пакеты проекта, и класс, наследующий базовый класс фреймворка, получает
только те уровни, что объявлены внутри анализируемого пути. Предок, которого не
удалось загрузить, считается корневым классом --- ровно как и тот, которого
вообще не удалось найти; отдельного значения «неизвестно» в отчёте нет.
