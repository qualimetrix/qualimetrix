...
**Маппинг в оценку здоровья:** Измерение `health.maintainability` растягивает естественный диапазон MI (обычно 70--85 для нормального кода) в более широкий диапазон здоровья. Формула `clamp((MI_avg - 40) x 1.667, 0, 100)` отображает MI=40 в здоровье 0, а MI=100 в здоровье 100, что даёт более дифференцированные оценки здоровья.
