...
Ошибки валидации появляются в выводе анализатора как диагностики `@qmx-threshold` и несут стабильные коды (`warning_exceeds_error`, `error_exceeds_warning`, `error_not_supported` и т.д.) для перекрёстных ссылок в машинно-читаемом выводе.
