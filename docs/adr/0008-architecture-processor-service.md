...
`Qualimetrix\Architecture\Processing\ArchitectureProcessor` (namespace per [ADR 0010](0010-architecture-vertical-slice.md)) is a scoped DI service that owns the complete rules-pipeline lifecycle: holding the prepared configuration, performing template expansion, binding the dependency graph to the registry, and answering per-class membership queries. All consumers route through it.
