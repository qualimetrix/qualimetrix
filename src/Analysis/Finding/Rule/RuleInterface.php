...
interface RuleInterface extends RuleDefinitionInterface
{
     ...
     * constructor, none static, and no `static` variable in a method. Two
     * calls with the same context must answer the same. A write through an
     * injected collaborator is allowed only when repeating the call leaves the
     ...
     * first half by class name, reading the source for writes to `$this`
     * outside the constructor; a write through a method of an object a
     * property holds and the collaborator half are enforced by nothing.
     *
     * @return list<Finding>
     */
