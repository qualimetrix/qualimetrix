...
Обычный файл или имя, под которым ещё ничего нет, пишется рядом с целью и
переименовывается поверх неё, поэтому читатель никогда не видит половину
отчёта; заменённый файл сохраняет свои права. Символическая ссылка
разрешается до файла, на который указывает, и остаётся ссылкой. Всё прочее,
что уже существует, — `/dev/stdout`, `/dev/null`, именованный канал,
подстановка процесса вроде `>(gzip > report.json.gz)` — пишется на месте.
Цель, в которую нельзя записать, отклоняется с кодом 3 до начала анализа:
каталог или имя, оканчивающееся на `/`, файл в каталоге, которого нет или
который недоступен для записи, или файл, недоступный для записи. Запись, не
удавшаяся уже после прогона, тоже завершает его с кодом 3.
...
[`--output`](#--output--o): каталог или имя, оканчивающееся на `/`, файл в
каталоге, которого нет или который недоступен для записи, и недоступный для
записи файл отклоняются с кодом 3 до чтения первого файла; символическая
ссылка, `/dev/stdout` и именованный канал записываются насквозь, а не
заменяются.
