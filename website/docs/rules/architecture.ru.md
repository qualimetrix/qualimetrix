...
- **Он судится только на прогоне, который способен судить.** Как и прочие
  ...
  и в `autoload-dev`. На более узком прогоне и на проекте, чей манифест ничего
  не объявляет в секциях, которые учитывает прогон, канал молчит.
...
`architecture.unreachable-layer` публикуется один раз на каждый объявленный слой — или на каждый конкретный инстанс, развёрнутый из шаблона, — чьи паттерны не совпали **ни с одним классом, ни с одним концом ребра зависимости** за прогон **и** который не мог бы владеть ни одним анализируемым классом, чьё назначение прогон оставил под сомнением. Слой, который владел бы анализируемым классом, если бы его убрал неразрешённый `exclude:` более раннего слоя, не объявляется не совпавшим ни с чем — к такому выводу прогон не пришёл. Не объявляется и слой, на который прогон не смог ответить об анализируемом классе, пока хоть один тип, названный его критериями `attributes` / `implements` / `extends`, прогону известен: объявлен в анализируемых путях, объявлен самим PHP или встретился на любом конце ребра зависимости. Оба слоя называет [`architecture.doubted-assignment`](#doubted-assignment). Неизвестного прогону типа для этого мало: класс, чей родитель лежит вне анализируемых путей, оставляет без ответа любой такой критерий, в том числе критерий с опечаткой, и на любом проекте, где анализируемый класс наследует vendor-код, опечатка иначе не нашлась бы никогда. Символ вне анализируемых путей тоже ни один слой от этой диагностики не спасает: прогон такой символ не читает. Находка говорит, что осталось за скобками, теми словами, которые для этого верны: о скольких символах слой не смог ответить и — если он не называет ни одного известного прогону типа — какие это типы: опечатка или тип, достижимый только через код, который прогон не анализировал, и тогда вопрос решает расширение `paths:`; либо со сколькими символами вне анализируемых путей совпали его критерии, пока их держит более ранний слой через `exclude:`, на который прогон не может ответить. Поэтому критерий, называющий тип, до которого доходит только vendor-цепочка, — `implements: ['Doctrine\Persistence\ObjectRepository']` над репозиториями, наследующими `ServiceEntityRepository`, когда анализируемый код нигде не называет этот интерфейс, — сообщается; чтобы слой не сообщался, достаточно назвать этот тип где-нибудь в анализируемом коде или расширить `paths:` до пакета. Это диагностика конфигурации (см. врезку в разделе [Режимы покрытия](#режимы-покрытия-coverage)): она валит прогон безусловно, как только срабатывает, и не настраивается, не бейзлайнится и не подавляется `@qmx-ignore`. Три возможные причины:
