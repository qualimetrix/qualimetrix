...
Анализ объединяет входные данные Halstead, вычисление MI и пороговую политику
в один конвейер поддерживаемости, поэтому оценка и подтверждающие её метрики
...
- **Число statements метода** -- независимый от форматирования семантический размер метода
...
!!! tip "Совет"
    Опция `minStatements` (по умолчанию: 10) отфильтровывает тривиально маленькие методы. Простые геттеры и сеттеры давали бы экстремальные значения MI, не имеющие практического смысла. Скорректируйте этот параметр, если получаете слишком много ложных срабатываний на коротких методах.
...
- **LOC-вход** = `size.method-statement-count`, независимое от форматирования число исполняемых и control-flow statements
...
**Маппинг в оценку здоровья:** Измерение `health.maintainability` использует штрафную формулу, учитывающую среднее MI (базовое качество), 5-й перцентиль MI (основной дифференциатор для проблемных методов) и минимум MI (экстремальные выбросы). Этот многокомпонентный подход обеспечивает хорошую дифференциацию между проектами — от качественных библиотек (оценка ~95) до сложных фреймворков (оценка ~48). Подробности и настройка — в разделе [Оценки здоровья](../reference/health-scores.md).

!!! info "Отклонение от оригинальной спецификации: statement-count"
    Qualimetrix использует для size-члена формулы отдельную метрику `size.method-statement-count`. Она считает семантические executable/control-flow statements, а не физические строки или старую Halstead-метрику `methodLoc`. Это делает MI стабильным к форматированию, но меняет существующие значения MI, агрегаты, health scores и baseline.

!!! info "Halstead Volume: семантический подход"
...
| `minStatements` | `10`         | Пропускать методы с меньшим числом statements |
...
rules:
  maintainability.mi:
    threshold: 20   # warning=20, error=20 → все нарушения ниже 20 становятся ошибками
```
