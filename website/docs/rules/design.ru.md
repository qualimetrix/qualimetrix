...
!!! info "Детали реализации"
    ...
    вашего кода, учитывается только если её может загрузить установка,
    выполняющая анализ, и глубина обрывается на первом предке, которого она
    загрузить не может.

    На практике это значит, что один и тот же класс может получить разную оценку
    в зависимости от способа установки инструмента. Установленный как
    зависимость анализируемого проекта, `qmx` делит с ним `vendor/` и видит всю
    цепочку. Запущенный из автономной установки --- Docker-образ, phar,
    `composer global require` --- он не видит пакеты проекта, и класс,
    наследующий базовый класс фреймворка, получает только те уровни, что
    объявлены внутри анализируемого пути. Предок, которого не удалось
    загрузить, считается корневым классом --- ровно как и тот, которого вообще
    не удалось найти; отдельного значения «неизвестно» в отчёте нет.

    Глубина, выводимая из самого анализируемого пути, от этого не зависит: когда
    вся цепочка внутри того, что вы анализируете, значение от установки не
    зависит.
