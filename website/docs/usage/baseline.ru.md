...
Запись baseline становится "устаревшей", когда она не встретилась в прогоне: её находка исправлена либо конфигурация перестала её порождать.

Устаревшие записи **только сообщаются**. Они никогда не роняют прогон и никогда не отключают
остальные записи: при используемой идентичности записи исправление одной находки на символе делает
устаревшей ровно её, и падение на этом означало бы наказание за улучшение.

Автоматически ничего не удаляется. `baseline:cleanup` удаляет записи, чьего файла больше нет:
...
### Вывод того, что скрывают теги подавления

Чтобы увидеть нарушения, подавленные тегами `@qmx-ignore`:

```bash
bin/qmx check src/ --no-suppression-annotations
```

Это меняет отчёт, а не baseline. Baseline измеряет то, что оставляют
конфигурация и аннотации, поэтому нарушение, убранное тегом `@qmx-ignore`,
никогда не записывается в baseline и никогда с ним не сравнивается. Под этим
флагом такое нарушение выводится со своей собственной серьёзностью и никогда не
повышается до ошибки.
...
{
    /**
     * @qmx-threshold complexity.cyclomatic warning=25 error=50 -- Легаси-машина состояний
...
```
@qmx-threshold <правило> <число> [-- <причина>]
@qmx-threshold <правило> warning=<число> [error=<число>] [-- <причина>]
@qmx-threshold <правило> error=<число> [warning=<число>] [-- <причина>]
```

- Числа должны быть неотрицательными целыми или десятичными. Короткая форма задаёт одно значение для warning и error
- Явная форма принимает только общие ключи `warning` и `error`: один или оба, в любом порядке. Произвольные имена опций из YAML или `--rule-opt` не поддерживаются
- Необязательная непустая причина должна следовать после `--` или длинного тире (`—`); текст без разделителя невалиден
- Рекомендуются точные имена правил. Префиксы вроде `complexity` и wildcard `*` поддерживаются, но пропускают валидацию конкретного правила, поскольку могут совпасть с правилами с разной семантикой порогов
- На одном символе можно использовать несколько тегов `@qmx-threshold` для разных правил
- Переопределение на классе применяется к проверкам внутри класса, включая его методы
- Переопределение на методе действует только на этот метод. При пересечении областей выбирается наименьший диапазон исходного кода; при равных диапазонах побеждает первое извлечённое переопределение
...
### Семантика переопределений по правилам

Большинство правил принимают стандартную форму «порог warning ≤ порог error» — всё, что выше warning, — это warning, выше error — это error. Некоторые правила используют другую семантику, потому что их метрика интерпретируется иначе. Аннотации валидируются по каждому правилу на этапе парсинга:

- **Инвертированные пороги** (`maintainability.index`, `design.type-coverage`) — большее значение метрики *лучше*, поэтому переопределение должно использовать `warning ≥ error`. Пример: `@qmx-threshold maintainability.index warning=50 error=30` срабатывает как warning, когда MI падает ниже 50, и как error — ниже 30.
- **Независимые оси** (`design.data-class`) — `warning` и `error` задают несвязанные метрики: warning соответствует `wocThreshold` (минимальный Weight of Class, высокий = больше публичной поверхности), error — `wmcThreshold` (максимальный Weighted Method Count, низкий = простые методы). Значения независимы; W > E так же валидно, как W < E.
- **Только warning** (`design.god-class`) — у правила одна настраиваемая величина (`minCriteria`); вторая половина аннотации не имеет аналога. Используйте короткую форму `@qmx-threshold design.god-class N` (задаёт warning) или явную `warning=N`. Указание `error=N` отклоняется с понятной диагностикой, чтобы отброшенное значение не стало сюрпризом.

Ошибки валидации появляются в выводе анализатора как диагностики `@qmx-threshold` и несут стабильные коды (`warning_exceeds_error`, `error_exceeds_warning`, `error_not_supported` и т.д.) для перекрёстных ссылок в машинно-читаемом выводе. Коды попадают в `violationCode: annotation.invalid-threshold.<код>` в выводе JSON/SARIF/Checkstyle.

!!! note "Частичные переопределения сохраняют дефолт второй половины"
    Валидация применяется к значениям, которые вы написали, — а не к итоговому Options после слияния с дефолтами. Если задана только одна половина (`@qmx-threshold complexity.cyclomatic warning=25`), правило сохраняет дефолтное значение `error`, и такая комбинация может оказаться в состоянии, которое валидатор отклонил бы, если бы вы написали обе половины явно. Для настройки обоих порогов предпочитайте явную форму.
