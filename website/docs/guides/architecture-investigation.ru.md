...
Оценки здоровья и списки нарушений говорят, *что* пространство имён запутано;
отрисованный граф показывает *как именно*. Экспортируйте его и передайте Graphviz:

```bash
# Формат DOT, сгруппировано по пространствам имён (по умолчанию)
bin/qmx graph:export src/ -o graph.dot
dot -Tsvg graph.dot -o graph.svg

# Формат JSON для собственных инструментов (агрегированный список смежности с метаданными)
...
`--no-clusters`, чтобы убрать группировку по пространствам имён, если кластеры
скрывают больше, чем показывают. Полное описание опций -- в разделе
[Опции CLI > graph:export](../usage/cli-options.ru.md#graphexport).

Для одного класса вместо всего графа есть более узкий вопрос --
`debug:layer-assignment` отвечает, к какому слою архитектуры разрешился класс
и какие ещё объявленные слои тоже могли бы его затронуть. См.
[Инспекция назначения слоя для одного класса](../rules/architecture.ru.md#debug-layer-assignment).
...
Правило срабатывает, когда мало что из публичного интерфейса класса несёт
поведение: низкий WOC вместе с низким WMC. Интерфейсы, абстрактные классы,
классы исключений и классы без свойств исключаются сразу; трейты — нет,
поскольку трейт с полями и их аксессорами это data-класс, размазанный по
единице переиспользования.

Большую часть того, что вы увидите, дают две популяции:

- **Данные по замыслу** — записи и value-объекты без `readonly`, держатели
  опций с публичными полями, структуры внешнего формата, повторяющие чужой
  payload. Это истинные срабатывания; вопрос лишь в том, выражен ли замысел в
  коде.
- **Имя как у аксессора, работа как у метода** — `is*`/`has*`/`get*`, которые
  сопоставляют, ищут или вычисляют, а не отдают поле. Принадлежность к
  аксессорам решается по имени, поэтому такие методы работают против WOC.
  Классу из трёх публичных членов достаточно двух таких имён, чтобы встать
  ровно на порог.

Используйте `excludeReadonly: true` и `excludePromotedOnly: true` для кодовых баз с PHP 8.2+ DTO. Если обычный класс намеренно является записью, лучше пометить его `readonly`, чем подавлять через `@qmx-ignore design.data-class`, — тогда замысел читается из кода. Для второй популяции лучшим лечением обычно оказывается переименование метода в то, что он делает.
...
- **`data-class`** -- намеренные записи лучше помечать `readonly`, чем подавлять
- **`identical-subexpression`** -- исключайте сгенерированные файлы через `--exclude` или baseline
- **`empty-catch`** -- `foreach`, перебирающий кандидатов до первого успешного, и так не помечается; любое другое намеренное игнорирование подавляйте через `@qmx-ignore code-smell.empty-catch` с причиной -- комментарий внутри `catch` находку не снимает
...
| `worstClasses[].metrics.{coupling.cbo, cohesion.tcc, complexity.wmc, maintainability.mi.avg}` | Диагностика на уровне класса                                     |
