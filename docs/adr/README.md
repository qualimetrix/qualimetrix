...
- [0073 — DIT Reads the Ancestors It Measures](0073-dit-reads-the-ancestors-it-measures.md) — depth beyond the analysed path is followed by parsing the project's own sources, never by loading them; the port is Design's and the adapter is Infrastructure's.
