...
!!! info "Работает для любого правила, включая `architecture.*`"
    `exclude_namespaces` и `exclude_paths` извлекаются и применяются на уровне фреймворка для
...
**Видимость:** в отличие от `@qmx-ignore`, это подавление по умолчанию происходит незаметно —
ничто в стандартном выводе не намекает на то, что нарушения были отброшены. Запустите с `-v`,
чтобы увидеть разбивку по правилам — сколько нарушений подавлено таким образом (отдельно для
`exclude_namespaces` и `exclude_paths`), и добавьте `--show-suppressed`, чтобы также вывести
