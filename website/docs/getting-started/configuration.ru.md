...
канала**, `X.*` для строгих потомков `X` либо явная пара `ruleName#violationCode`; см.
[«Селекторы правил и каналов»](#селекторы-правил-и-каналов) ниже. Голый префикс вроде `health` —
ошибка, а не группа: пишите `health.*`. Точный `health.cohesion` не затрагивает соседний канал
`health.coupling`.
...
rules:
  computed.health:
    exclude_namespace_channels:
      # тот же канал, записанный полностью; по действию идентично `health.cohesion`
      'computed.health#health.cohesion':
        - App\Metrics\Coupling
```

Здесь пара избыточна: ключ и так лежит под именем правила, оба написания говорят одно и то
же. Она принимается ради единства грамматики, а не потому, что расширяет охват.
