...
!!! info "Отклонение от оригинальной спецификации"
    ...
    отдельности увеличивать число изолированных компонент. Метод, чьё тело —
    единственный `return` класс-константы, засчитывается независимо от того,
    какому классу принадлежит константа: `self::X`, `static::X`, `parent::X`,
    `Foo::X` у импортированного класса и кейс перечисления вроде
    `Suit::Hearts` — все они не читают состояние экземпляра, поэтому все
    распознаются как константные выражения. Динамическое имя класса или
    константы (`$var::X`, `Foo::{$name}`) разрешается в рантайме и константным
    выражением не считается. Конструктор и
...
- **Классы с 0 или 1 отслеживаемым публичным методом** вовсе не получают значения TCC и LCC: при менее чем двух методах нет ни одной пары, и отношение не определено (0/0). Метрики в выводе класса отсутствуют, а не равны 1.0.
- **Агрегаты уровня пространства имён и проекта** (`cohesion.tcc.avg`, `cohesion.tcc.min` и аналоги для LCC) вычисляются только по классам, у которых значение есть; `.count` показывает, по скольким. Там, где TCC нет, оценка `health.cohesion` уровня класса подставляет 0.5 для класса менее чем с шестью методами и 0 — несвязный край — для более крупного (см. [Оценки здоровья](../reference/health-scores.md)).

!!! info "Deviation from original spec"
    Классы, **не объявляющие свойств экземпляра**, тоже не получают значения TCC и LCC, даже при двух и более отслеживаемых методах. По формуле Bieman & Kang у такого класса есть пары методов, но нет общих переменных экземпляра, так что TCC = LCC = 0. Qualimetrix считает связность через общее состояние неприменимой к классу без состояния, а не сообщает его максимально несвязным, — иначе каждый сервис без состояния тянул бы вниз средние значения пространства имён.
